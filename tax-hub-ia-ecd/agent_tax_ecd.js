/**
 * ============================================
 * AGENT TAX CONSULT SPED ECD - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'ecd',
    agentName: 'Agente Tax - Consultor SPED ECD',
    apiContext: 'SPED_ECD',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre SPED ECD, selecione novamente o agente.'
  };
   ...
   * O conte�do foi retirado do 'agent_tax_ecd.html' original.
   */
  function adicionarMensagemInicial() {
    // Corrigimos a codifica��o (ex: Ola!)
    const mensagemInicial = `Ol�! Eu sou o seu assistente para <strong>SPED ECD</strong>.
      <br><br>
      Posso apoiar com:
      <ul>
        <li>Valida��o de layout, blocos e registros obrigat�rios;</li>
        <li>Regras de substitui��o e retifica��es da ECD;</li>
        <li>Tratamento de lan�amentos, refer�ncia cruzada e plano de contas referencial;</li>
        <li>Integração com ERPs, certificados digitais e PVA;</li>
        <li>Boas pr�ticas de controllership para auditorias e fiscaliza��es.</li>
      </ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA SPED ECD ***
    const placeholders = [
      'Descreva sua d�vida sobre SPED ECD...',
      'Como retificar uma ECD?',
      'O que � o Bloco J (Demonstra��es)?',
      'Preciso assinar a ECD com certificado digital?',
      'O que � o plano de contas referencial?'
    ];
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA SPED ECD ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-book' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-book'; // �cone do ECD
    avatarDiv.appendChild(avatarIcon);
   ...
   * uma resposta pr�-definida baseada nos t�picos do ECD.
   *
  ...
  function gerarRespostaBot(mensagem) {
    ...
    // T�pico 1: Retifica��o / Substitui��o
    if (texto.includes('retificar') || texto.includes('substituir') || texto.includes('corrigir')) {
      return `A **retifica��o da ECD** (Escritura��o Cont�bil Digital) � um processo complexo.
      
      **Regras principais:**
      <ul>
        <li>**Substitui��o:** A ECD autenticada S� pode ser substitu�da (cancelada e enviada uma nova) se a nova escritura��o for enviada **ANTES** da entrega da ECF (Escritura��o Cont�bil Fiscal) do mesmo per�odo.</li>
        <li>**Retifica��o (ap�s ECF):** Se a ECF j� foi entregue, a ECD n�o pode mais ser substitu�da. Corre��es devem ser feitas via lan�amento cont�bil extempor�neo (Registro I200) na ECD do per�odo seguinte.</li>
        <li>N�o � poss�vel retificar uma ECD que j� foi autenticada pela Junta Comercial se o erro n�o for de natureza t�cnica.</li>
      </ul>`;
    }

    // T�pico 2: Blocos e Layout
    if (texto.includes('blocos') || texto.includes('layout') || texto.includes('bloco j') || texto.includes('bloco i')) {
      return `O layout da ECD � dividido em blocos. Os mais importantes s�o:
      
      <ul>
        <li>**Bloco 0:** Abertura, Identifica��o e Refer�ncias.</li>
        <li>**Bloco I:** Lan�amentos Cont�beis (o "cora��o" da ECD, com o Di�rio). Inclui I050 (Plano de Contas), I051 (Plano Referencial) e I200 (Lan�amentos).</li>
        <li>**Bloco J:** Demonstra��es Cont�beis. Onde entram o Balan�o Patrimonial (J100) e a DRE (J150). � obrigat�rio para a maioria das empresas.</li>
        <li>**Bloco K:** Conglomerados (geralmente para bancos).</li>
        <li>**Bloco 9:** Encerramento do Arquivo Digital.</li>
      </ul>
      O **Bloco J** (Demonstra��es) � frequentemente fonte de erros de valida��o se os valores n�o baterem com os saldos do Bloco I.`;
    }

    // T�pico 3: Plano de Contas Referencial
    if (texto.includes('plano de contas') || texto.includes('referencial') || texto.includes('i051')) {
      return `O **Plano de Contas Referencial** (Registro I051) � obrigat�rio na ECD.
      
      **Como funciona:**
      <ol>
        <li>A empresa informa seu plano de contas societ�rio (anal�tico) no **Registro I050**.</li>
        <li>No **Registro I051**, a empresa deve fazer o "DE-PARA" (mapeamento) de cada conta do I050 para uma conta do Plano Referencial da Receita Federal (RFB).</li>
        <li>Esse mapeamento � crucial, pois � ele que a Receita Federal utiliza para "entender" a contabilidade da empresa e pr�-preencher a ECF (Escritura��o Cont�bil Fiscal).</li>
      </ol>
      Erros no I051 s�o a principal causa de problemas na recupera��o da ECD dentro da ECF.`;
    }
    
    // T�pico 4: Assinatura / Certificado
    if (texto.includes('assinar') || texto.includes('certificado') || texto.includes('pva')) {
      return `A ECD exige m�ltiplas assinaturas digitais (certificados A1 ou A3) para ser v�lida:
      
      **Assinaturas Obrigat�rias:**
      <ul>
        <li>**Contador/Contabilista:** � obrigat�ria a assinatura do profissional cont�bil (com seu **e-CPF**).</li>
        <li>**Representante Legal:** � obrigat�ria a assinatura do representante legal da empresa (com **e-CNPJ** ou e-CPF do representante).</li>
      </ul>
      
      O **PVA (Programa Validador e Assinador)** � o software da Receita Federal usado para validar a estrutura do arquivo, assinar e transmitir a ECD.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Sou o seu assistente para O <strong>SPED ECD</strong>.
      <br><br>
      Posso apoiar com:
      <ul>
        <li>Valida��o de layout, blocos e registros obrigat�rios;</li>
        <li>Regras de substitui��o e retifica��es da ECD;</li>
        <li>Tratamento de lan�amentos e plano de contas referencial;</li>
        <li>Integração com ERPs, certificados digitais e PVA.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente SPED ECD</strong><br><br>
