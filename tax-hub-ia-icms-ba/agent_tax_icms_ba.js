/**
 * ============================================
 * AGENT TAX CONSULT ICMS BA - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ba', 
    agentName: 'Agente Tax - Consultor ICMS - BA',
    apiContext: 'ICMS_BA',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - BA, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - BA:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente dedicado ao <strong>ICMS da Bahia</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
        <li>Benefícios fiscais (DESENVOLVE, SUDES) e programas de desenvolvimento;</li>
        <li>Substituição tributária, diferencial de alíquotas e operações interestaduais;</li>
        <li>Obrigações acessórias estaduais (EFD ICMS/IPI, DMA, GIA-ST, etc.);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre o ICMS da Bahia...',
      'O que é o DESENVOLVE?',
      'Qual a alíquota interna de ICMS na Bahia?',
      'Como funciona a ST para autopeças na BA?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Bahia (BA).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('desenvolve') || (texto.includes('benefício') && texto.includes('bahia'))) {
      return `O **Programa DESENVOLVE** (Decreto nº 8.205/2002) é um dos principais incentivos fiscais da Bahia para o setor industrial.
      
      **O que oferece:**
      <ul>
        <li>Crédito presumido de ICMS, resultando em uma carga tributária efetiva reduzida (ex: 3% a 0,6%).</li>
        <li>O percentual de crédito varia conforme o tipo de indústria, localização (interior vs. capital) e outros fatores.</li>
        <li>Diferimento (adiamento) do ICMS sobre a importação de máquinas (ativo imobilizado) e aquisição de matéria-prima.</li>
      </ul>
      A adesão depende de um projeto de viabilidade técnica e econômica aprovado pelo Estado.`;
    }

    if (texto.includes('st') || (texto.includes('substituição') && texto.includes('bahia'))) {
      return `O **ICMS-ST na Bahia** é complexo e possui várias particularidades:
      
      **Pontos de Atenção:**
      1.  **MVA/IVA-ST:** A Bahia utiliza suas próprias MVAs (Margens de Valor Agregado), que são frequentemente atualizadas por Portarias e Decretos. �? crucial consultar a legislação mais recente para o produto específico.
      2.  **Ressarcimento:** O processo de ressarcimento (para saídas interestaduais) ou complemento (quando a venda final é maior que a base da ST) é regulamentado pelo RICMS/BA e exige controle rigoroso.
      3.  **Antecipação Parcial/Total:** A Bahia exige a "Antecipação Parcial" ou "Antecipação Total" do ICMS em muitas entradas interestaduais, o que funciona de forma similar à ST.`;
    }

    if (texto.includes('difal') || texto.includes('diferencial de alíquota')) {
      return `O **DIFAL na Bahia** (para consumidor final não contribuinte, EC 87/2015) exige atenção especial:
      
      **Alíquota Interna:** A alíquota interna padrão na Bahia é de **19%**. (Verificar se houve mudanças recentes, pois em 2023/2024 muitos estados alteraram suas alíquotas).
      
      **FCECEP:** A Bahia exige o recolhimento do **FCECEP** (Fundo Estadual de Combate e Erradicação da Pobreza), que é um adicional de **2%** sobre a alíquota interna na maioria das operações.
      
      **Exemplo:** Venda de SP (12%) para BA (19% + 2% FCECEP) para consumidor final:
      <ul>
        <li>DIFAL (ICMS): 7% (19% - 12%) para a Bahia.</li>
        <li>FCECEP: 2% para a Bahia (geralmente pago na mesma guia do DIFAL ou em guia separada, dependendo da operação).</li>
      </ul>`;
    }

    // Resposta Padrão (corrigida)
    return `Sou o especialista em <strong>ICMS da Bahia</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ICMS - BA</strong><br><br>
