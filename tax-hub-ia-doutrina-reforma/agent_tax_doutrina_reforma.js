/**
 * ============================================
 * AGENTE DOUTRINA REFORMA TRIBUT�RIA - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-reforma',
    agentName: 'Agente Doutrinadores - Reforma Tributária',
    apiContext: 'DOUTRINA_REFORMA',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Reabra o agente para novas consultas sobre a reforma tribut�ria.'
  };

  // Lista de autores e obras (do JS original)
  const BIBLIOGRAFIA = [
    {
      autor: 'Renaldo R. Junior',
      obra: 'Reforma Tributária - Lei Complementar no 214/2025 Comentada', // Assumindo LC fict�cia
      destaque: 'Análise pr�tica com foco em compliance, governan�a e cronograma de implanta��o.'
    },
    {
      autor: 'Felipe Duque',
      obra: 'Reforma Tributária Esquematizada',
      destaque: 'Vis�o esquem�tica de um procurador da Fazenda Nacional, com mapas mentais e tabelas comparativas.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Quais cap�tulos Renaldo dedica a compliance e governan�a?',
    'Como Felipe Duque apresenta o cronograma de implanta��o do IBS e CBS?',
    'Sugest�es para comunicar a reforma ao conselho de administra��o?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Ol�! Estou pronto para guiar seus estudos sobre a <strong>reforma tribut�ria</strong>.
      <br><br>
      <ul>
        ${listaAutores}
      </ul>
      Auxilio em compliance, comunica��o com stakeholders, cronogramas de implanta��o e an�lise de riscos. Qual parte da reforma voc� quer examinar?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-arrows-rotate' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-arrows-rotate'; // �cone da Reforma
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('renaldo') || texto.includes('compliance') || texto.includes('lc 214')) {
      return `**Renaldo R. Junior** detalha os artigos da LC (ainda pendente), destaca cronogramas de redu��o de PIS/COFINS e aumento gradativo de CBS/IBS, al�m de orienta��es de compliance para TI, fiscal e contabilidade.`;
    }

    if (texto.includes('felipe') || texto.includes('duque') || texto.includes('esquematizado')) {
      return `**Felipe Duque** utiliza quadros sin�ticos para explicar impostos substitu�dos, distribui��o federativa e regimes espec�ficos. Excelente para apresenta��es internas e treinamentos.`;
    }

    if (texto.includes('comunica��o') || texto.includes('conselho') || texto.includes('governan�a')) {
      return `Combine a parte pr�tica de Renaldo (planos de implementa��o) com os esquemas de Duque para montar dashboards de acompanhamento, definindo respons�veis por projeto e controles de riscos.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie um tema (compliance, cronograma, comunica��o, simula��es) e trago os resumos e refer�ncias adequados das obras de Renaldo R. Junior e Felipe Duque.`;
  }
