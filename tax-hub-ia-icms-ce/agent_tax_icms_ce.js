/**
 * ============================================
 * AGENT TAX CONSULT ICMS CE - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ce', 
    agentName: 'Agente Tax - Consultor ICMS - CE',
    apiContext: 'ICMS_CE',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - CE, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - CE:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente dedicado ao <strong>ICMS do Ceará</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
        <li>Benefícios fiscais (FDI), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substituição tributária, diferencial de alíquotas e operações interestaduais;</li>
        <li>Obrigações acessórias estaduais (EFD ICMS/IPI, DIEF, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre o ICMS do Ceará...',
      'Como funciona o FDI Ceará?',
      'Qual a alíquota interna de ICMS no Ceará?',
      'Como funciona a ST para Eletrodomésticos no CE?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Ceará (CE).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('fdi') || (texto.includes('benefício') && texto.includes('ceará'))) {
      return `O **FDI (Fundo de Desenvolvimento Industrial do Ceará)** é o principal programa de incentivo do estado.
      
      **O que oferece:**
      <ul>
        <li>Diferimento (adiamento) do ICMS na importação de bens de capital (ativo imobilizado).</li>
        <li>Diferimento do ICMS na aquisição interna de bens de capital.</li>
        <li>Crédito presumido do ICMS em operações próprias, podendo chegar a 75% do imposto devido.</li>
      </ul>
      A adesão é complexa e exige um projeto aprovado pela ADECE (Agência de Desenvolvimento do Estado do Ceará).`;
    }

    if (texto.includes('st') || (texto.includes('substituição') && texto.includes('ceará'))) {
      return `O **ICMS-ST no Ceará** segue as regras gerais dos convênios CONFAZ, mas possui particularidades:
      
      **Pontos de Atenção:**
      1.  **MVA/IVA-ST:** O Ceará utiliza suas próprias Margens de Valor Agregado (MVA), que são definidas em Decretos e Instruções Normativas específicas.
      2.  **Carga Líquida:** O Ceará é famoso pelo sistema de "carga líquida" em certas operações, que substitui a apuração normal do ICMS (débito e crédito) por um percentual fixo sobre a entrada ou saída.
      3.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para saídas interestaduais) é detalhado na legislação local e exige validação de arquivos.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Ceará possui o **FECOP (Fundo Estadual de Combate à Pobreza)**.
      
      **Como funciona:**
      <ul>
        <li>�? um adicional de **2%** sobre a alíquota do ICMS.</li>
        <li>Incide sobre produtos e serviços considerados "supérfluos" (ex: bebidas alcoólicas, cigarros, perfumes, energia elétrica acima de certa faixa, etc.).</li>
        <li>Portanto, a alíquota interna padrão do Ceará é **18%**, mas para esses produtos, a alíquota final se torna **20%** (18% + 2% FECOP).</li>
        <li>O valor do FECOP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padrão (corrigida)
    return `Sou o especialista em <strong>ICMS do Ceará</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ICMS - CE</strong><br><br>
