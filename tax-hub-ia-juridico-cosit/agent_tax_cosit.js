...
(() => {
  ...
  const CONFIG = {
    agentId: 'cosit',
    agentName: 'Agente Tax - Consultor COSIT',
    apiContext: 'JURIDICO_COSIT',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Reabra o agente COSIT quando quiser revisar novas solu��es de consulta.'
  };
  ...
  function adicionarMensagemInicial() {
    const mensagemInicial = `Bem-vindo ao **agente especializado em Solu��es de Consulta COSIT**.
      <br><br>
      Estou pronto para:
      <ul>
        <li>Localizar respostas vinculantes por n�mero ou tema</li>
        <li>Comparar entendimentos entre consultas e leis posteriores</li>
        <li>Destacar efeitos vinculantes e aplica��o regional</li>
        <li>Orientar procedimentos para formular uma consulta</li>
        <li>Avaliar reflexos em PIS, Cofins, IRPJ, CSLL e IPI</li>
      </ul>
      Qual decis�o da Receita Federal voc� quer analisar agora?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA COSIT (do JS original) ***
    const placeholders = [
      'Resumo da Solu��o de Consulta COSIT 183/2023?',
      'Existe entendimento sobre cr�ditos de PIS Cofins em freight?',
      'Quais consultas tratam de tributa��o de software SaaS?',
      'Diverg�ncias entre COSIT e Cosit Disit precisam de alinhamento?'
    ];
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA COSIT ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-file-signature' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-file-signature'; // �cone do COSIT
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('cr�dito') || texto.includes('pis') || texto.includes('cofins')) {
      return `A COSIT tem orientado que cr�ditos de **PIS/Cofins** dependem da essencialidade e relev�ncia.

**Resumo comum nas consultas:**
<ul>
  <li>Servi�os de freight podem gerar cr�dito se vinculados � opera��o de venda.</li>
  <li>Despesas com marketing tendem a n�o ser aceitas.</li>
  <li>Insumos para software SaaS exigem demonstrar uso direto na presta��o.</li>
  <li>Exige-se controle documental de notas fiscais e contratos.</li>
</ul>
Verifique as solu��es 313/2019 e 4/2022 para exemplos de aceita��o restritiva.`;
    }

    if (texto.includes('software') || texto.includes('saas') || texto.includes('servi�o digital')) {
      return `Em **solu��es de consulta sobre software**, a COSIT diferencia licen�a, SaaS e disponibiliza��o remota.

**Pontos recorrentes:**
<ul>
  <li>SaaS tratado como presta��o de servi�o com ISS, n�o IOF ou ICMS.</li>
  <li>Licen�a padr�o pode gerar IRRF quando pago ao exterior.</li>
  <li>Necessidade de registro no INPI n�o afasta car�ter de servi�o.</li>
  <li>Retenções de PIS/Cofins/CSLL sobre pagamentos a empresas nacionais.</li>
</ul>
Consulte a solu��o 36/2023 para um panorama completo de SaaS.`;
    }

    if (texto.includes('procedimento') || texto.includes('formular') || texto.includes('prazo')) {
      return `Para **formular consulta COSIT**, siga estes passos:

<ol>
  <li>Protocolar na unidade da Receita Federal com formul�rio pr�prio.</li>
  <li>Descrever fatos com riqueza de detalhes e documentos de suporte.</li>
  <li>Demonstrar exist�ncia de d�vida leg�tima e aus�ncia de fiscaliza��o em curso.</li>
  <li>Aguardar resposta vinculante; cabem solicita��es complementares.</li>
</ol>
Enquanto pendente, suspende-se o prazo para recolhimento do tributo debatido.`;
    }

    if (texto.includes('diverg�ncia') || texto.includes('solu��o divergente') || texto.includes('disit')) {
      return `Quando h� **diverg�ncia entre solu��es**, � poss�vel interpor recurso hier�rquico.

**Fluxo t�pico:**
<ul>
  <li>Identificar conflito entre solu��o COSIT e solu��o DISIT.</li>
  <li>Protocolar recurso demonstrando discrep�ncias de fato ou direito.</li>
  <li>COSIT pode editar solu��o definitiva ou ato declarat�rio interpretativo.</li>
  <li>At� l�, contribuinte pode seguir a solu��o mais favor�vel sem multa.</li>
</ul>
Registros de diverg�ncia s�o publicados no s�tio da Receita para acompanhamento.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Posso apoiar voc� em qualquer tema relacionado a **Solu��es de Consulta COSIT**:
<ul>
  <li>Pesquisas por n�mero, assunto ou setor econ�mico.</li>
  <li>Resumo executivo para decis�o r�pida.</li>
  <li>Avalia��o de impactos em cr�ditos e d�bitos fiscais.</li>
  <li>Compara��o com legislação e jurisprud�ncia posterior.</li>
  <li>Orientação para formular nova consulta.</li>
</ul>
Envie mais detalhes para um direcionamento preciso.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente COSIT</strong><br><br>
