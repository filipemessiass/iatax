/**
 * ============================================
 * AGENT TAX CONSULT ICMS AM - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-am', 
    agentName: 'Agente Tax - Consultor ICMS - AM',
    apiContext: 'ICMS_AM',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - AM, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - AM:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Amazonas</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (ZFM/SUFRAMA), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, DAM, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Amazonas...',
      'Como funciona a SUFRAMA?',
      'O que � o "corredor de importa��o" de AM?',
      'Qual a al�quota interna de ICMS-AM?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Amazonas (AM).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('suframa') || (texto.includes('zona franca') && texto.includes('manaus'))) {
      return `A **Zona Franca de Manaus (ZFM)**, administrada pela **SUFRAMA**, � o principal pilar do ICMS do Amazonas.
      
      **O que oferece:**
      <ul>
        <li>**Isen��o/Suspens�o do IPI:** Na importa��o e compra nacional de insumos.</li>
        <li>**Isen��o/Diferimento do ICMS:** Na importa��o de insumos e ativo imobilizado.</li>
        <li>**Cr�dito Est�mulo:** As ind�strias da ZFM geram um "Cr�dito Est�mulo" (cr�dito presumido) que pode chegar a 100% do ICMS devido na sa�da do produto industrializado.</li>
      </ul>
      Para usufruir, a empresa precisa ter um **Projeto Aprovado** na SUFRAMA e cumprir o Processo Produtivo B�sico (PPB).`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('amazonas'))) {
      return `O **ICMS-ST no Amazonas** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O Amazonas utiliza suas pr�prias MVAs (Margens de Valor Agregado), definidas na legislação interna (RICMS/AM).
      2.  **ZFM:** Produtos destinados � Zona Franca de Manaus (para industrializa��o ou revenda) geralmente t�m **suspens�o ou isen��o** do ICMS, o que **n�o se aplica a ST** em muitos casos. � crucial verificar o conv�nio do produto.
      3.  **Antecipa��o:** O Amazonas exige o ICMS Antecipado na entrada de mercadorias no estado, com regras espec�ficas de c�lculo.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Amazonas possui o **FECOP (Fundo Estadual de Combate e Erradica��o da Pobreza)**, chamado de Fundo de Promo��o Social (FPS).
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A al�quota interna padr�o no Amazonas � **20%**. Para esses produtos "sup�rfluos", a al�quota final se torna **22%** (20% + 2% FECOP).</li>
        <li>O valor do FECOP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Amazonas</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - AM</strong><br><br>
