/**
 * ============================================
 * AGENTE DOUTRINA PRE�O DE TRANSFER�NCIA - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-transferencia',
    agentName: 'Agente Doutrinadores - Preços de Transferência',
    apiContext: 'DOUTRINA_TRANSFERENCIA',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Retorne quando quiser revisar pre�os de transfer�ncia e a Lei 14596/23.'
  };

  // Lista de autores e obras (do JS original)
  const BIBLIOGRAFIA = [
    {
      autor: 'Marco Antonio Moreira Monteiro',
      obra: 'Preços de Transferência - O Sistema Brasileiro no Cen�rio Global',
      destaque: 'Analisa a Lei 14596/23 e compara com diretrizes da OCDE.'
    },
    {
      autor: 'Ramon Tomazela',
      obra: 'Manual dos Preços de Transferência',
      destaque: 'Passo a passo pr�tico para implementa��o de pol�ticas e documenta��o.'
    },
    {
      autor: 'Luis Eduardo Schoueri',
      obra: 'Preços de Transferência no Direito Tribut�rio Brasileiro',
      destaque: 'Abordagem te�rica e hist�rica dos m�todos brasileiros e internacionais.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Resumo do Monteiro sobre m�todos MCL e PRL?',
    'Quais planilhas o Ramon Tomazela recomenda para compliance?',
    'Como Schoueri explica a intera��o com conv�nios para evitar dupla tributa��o?'
  ];
  ...
  function adicionarMensagemInicial() {
    // Corrigimos a codifica��o (ex: A?A?) e melhoramos a formata��o para <li>
    const listaAutores = BIBLIOGRAFIA.map(
      (item) => `<li><strong>${item.autor}</strong> (${item.obra}): ${item.destaque}</li>`
    ).join('');

    const mensagem = `Bem-vindo! Aqui est�o os principais <strong>doutrinadores de pre�os de transfer�ncia</strong>.
      <br><br>
      **Refer�ncias recomendadas:**
      <ul>
        ${listaAutores}
      </ul>
      Posso apoiar na interpreta��o da Lei 14596/23, na configura��o de m�todos, na compara��o com OCDE e na prepara��o de dossi�s. Qual etapa do projeto voc� precisa destrinchar?`;

    adicionarMensagem('bot', mensagem);
  }
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA PRE�OS DE TRANSFER�NCIA ***
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-globe'; // �cone de Preços de Transferência
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    // Corrigindo a codifica��o da l�gica de resposta (ex: A?A?o -> ��o)
    if (texto.includes('monteiro') || texto.includes('marco')) {
      return `**Marco Antonio Moreira Monteiro** contrasta os m�todos tradicionais com os ajustes da Lei 14596/23 e detalha a intera��o com pilares BEPS. Ele recomenda criar matriz de decis�o considerando perfil da opera��o, disponibilidade de compar�veis e exig�ncias de safe harbors.`;
    }

    if (texto.includes('ramon') || texto.includes('tomazela')) {
      return `No **Manual dos Preços de Transferência**, **Ramon Tomazela** apresenta checklists operacionais, modelos de planilhas e orienta o uso de bases de dados para testar PIC, PRL e MCL. O autor destaca prazos para entregar o arquivo digital e efeitos do regime de transição 2023/2024.`;
    }

    if (texto.includes('schoueri') || texto.includes('te�rico')) {
      return `**Luis Eduardo Schoueri** discute fundamentos jur�dicos, rela��o com tratados e a compatibiliza��o entre princ�pios de plena compet�ncia e regras dom�sticas. Utilize seus cap�tulos para embasar pareceres e alinhar interpreta��es com jurisprud�ncia.`;
    }

    if (texto.includes('documenta��o') || texto.includes('dossi�') || texto.includes('ocde')) {
      return `Combine a orientação pr�tica de Tomazela com o cap�tulo de Monteiro sobre Master File e Local File, verificando o cronograma estabelecido pela Receita. A obra de Schoueri auxilia na justificativa jur�dica do m�todo escolhido quando houver aus�ncia de compar�veis dom�sticos.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie o nome do autor, d�vida sobre m�todo (PIC, PRL, MCL, MLT, MDL) ou impacto da Lei 14596/23, e apresento trechos recomendados e estrat�gias de compliance.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Preços de Transferência</strong><br><br>
