/**
 * ============================================
 * AGENT TAX CONSULT ICMS PA - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-pa', 
    agentName: 'Agente Tax - Consultor ICMS - PA',
    apiContext: 'ICMS_PA',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - PA, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - PA:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Par�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Par�...',
      'Quais s�o os benef�cios fiscais do Par�?',
      'Como funciona a ST para min�rio no PA?',
      'Qual a al�quota interna de ICMS-PA?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Par� (PA).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('benef�cio') || texto.includes('incentivo') || texto.includes('cr�dito presumido')) {
      return `O Par� possui diversos **Incentivos Fiscais** para atrair ind�strias, regulamentados pela Pol�tica de Incentivos ao Desenvolvimento Socioecon�mico.
      
      **Principais Benef�cios:**
      <ul>
        <li>**Cr�dito Presumido:** O principal mecanismo � a concess�o de cr�dito presumido sobre o ICMS devido nas sa�das, variando de 55% a 95% do imposto.</li>
        <li>**Diferimento:** Tamb�m � comum o diferimento (adiamento) do ICMS na importa��o de m�quinas (ativo imobilizado) e na aquisi��o de energia el�trica.</li>
        <li>**Condi��es:** A concess�o depende da atividade (industrial, agroneg�cio, etc.) e do local de instala��o.</li>
      </ul>
      A ades�o � formalizada atrav�s de um Termo de Acordo com a SEFA-PA.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('par�'))) {
      return `O **ICMS-ST no Par�** tem um ponto de aten��o muito importante: a **Antecipa��o Especial**.
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O Par� utiliza tanto MVAs de conv�nios/protocolos quanto MVAs pr�prias definidas no Anexo XIII do RICMS/PA.
      2.  **Antecipa��o Especial (Art. 114-A):** Na entrada de mercadorias de outros estados, o Par� exige o recolhimento do ICMS "Antecipado Especial" que corresponde � diferen�a entre a al�quota interna e a interestadual, *mesmo que a mercadoria n�o seja para consumidor final*.
      3.  **N�o-Cumulatividade:** O imposto pago como Antecipado Especial vira cr�dito para o adquirente no Par�.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Par� possui o **FECEP (Fundo Estadual de Combate � Pobreza)**, mas ele foi **extinto** em 2023.
      
      A Lei n� 10.028/2023 **revogou** o adicional de 2% que incidia sobre produtos como cigarros, bebidas alco�licas, energia el�trica, etc.
      
      **Al�quota Atual:** A al�quota interna padr�o do ICMS no Par� � de **19%** (desde 2024). N�o h� mais o adicional do FECOP na maioria das opera��es.`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Par�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais e incentivos regionais (cr�dito presumido, diferimento);</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e o ICMS Antecipado Especial;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA-ST, Dief, GNRE);</li>
        <li>Procedimentos de fiscaliza��o, concilia��o de documentos fiscais e regimes especiais.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - PA</strong><br><br>
