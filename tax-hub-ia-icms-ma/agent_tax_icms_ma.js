/**
 * ============================================
 * AGENT TAX CONSULT ICMS MA - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ma', 
    agentName: 'Agente Tax - Consultor ICMS - MA',
    apiContext: 'ICMS_MA',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - MA, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - MA:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente dedicado ao <strong>ICMS do Maranhão</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre o ICMS do Maranhão...',
      'Quais são os benefícios fiscais do MA?',
      'Qual a MVA da ST para combustíveis no Maranhão?',
      'Como funciona o FUMACOP?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Maranhão (MA).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('benefício') || texto.includes('incentivo') || texto.includes('mais empresas')) {
      return `O Maranhão possui o programa **MAIS EMPRESAS** (Lei nº 10.305/2015), que é um dos principais incentivos para atração de indústrias e centros de distribuição.
      
      **O que oferece:**
      <ul>
        <li>Crédito presumido de ICMS que pode chegar a **95%** do imposto devido nas operações próprias.</li>
        <li>O benefício é concedido mediante regime especial e varia conforme o tipo de atividade e investimento.</li>
        <li>Também pode incluir diferimento na aquisição de ativo imobilizado.</li>
      </ul>
      �? um regime muito atrativo para novas instalações no estado.`;
    }

    if (texto.includes('st') || (texto.includes('substituição') && texto.includes('maranhão'))) {
      return `O **ICMS-ST no Maranhão** tem como principal característica a utilização de **Pautas Fiscais** (Valores de Referência).
      
      **Pontos de Atenção:**
      1.  **Base de Cálculo:** Para muitos produtos, a base de cálculo da ST não é definida por uma MVA (Margem de Valor Agregado) sobre o preço do fornecedor, mas sim por um **valor fixo por unidade** (pauta) estabelecido pelo estado.
      2.  **MVA:** Quando não há pauta, o Maranhão utiliza as MVAs definidas em Protocolos e Convênios CONFAZ.
      3.  **Obrigações:** A apuração da ST e a entrega da EFD ICMS/IPI são cruciais e devem refletir essas bases de cálculo específicas.`;
    }

    if (texto.includes('fumacop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Maranhão possui o **FUMACOP** (Fundo Maranhense de Combate à Pobreza).
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A alíquota interna padrão do Maranhão é **20%**. Para esses produtos "supérfluos", a alíquota final se torna **22%** (20% + 2% FUMACOP).</li>
        <li>O valor do FUMACOP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padrão (corrigida)
    return `Sou o especialista em <strong>ICMS do Maranhão</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
        <li>Benefícios fiscais (MAIS EMPRESAS), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substituição tributária, diferencial de alíquotas e operações interestaduais;</li>
        <li>Obrigações acessórias estaduais (EFD ICMS/IPI, GIA, GNRE, entre outras);</li>
        <li>Procedimentos de fiscalização, conciliação de documentos fiscais e regimes especiais.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ICMS - MA</strong><br><br>
