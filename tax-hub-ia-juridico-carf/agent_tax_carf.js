...
(() => {
  ...
  const CONFIG = {
    agentId: 'carf',
    agentName: 'Agente Tax - Consultor CARF',
    apiContext: 'JURIDICO_CARF',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Para retornar ao estudo de jurisprud�ncia do CARF, basta abrir o agente novamente.'
  };
  ...
  function adicionarMensagemInicial() {
    const mensagemInicial = `Ol�! Eu sou o **consultor focado em CARF** do TaxHub IA.
      <br><br>
      Posso auxiliar voc� em:
      <ul>
        <li>Identificar ac�rd�os relevantes por mat�ria ou per�odo</li>
        <li>Consolidar fundamentos de votos e diverg�ncias</li>
        <li>Mapear efeitos do voto de qualidade e da Lei 14302</li>
        <li>Produzir sum�rios para recursos volunt�rios ou especiais</li>
        <li>Destacar penalidades, multas qualificadas e decad�ncia</li>
      </ul>
      Qual tema do contencioso administrativo federal voc� quer destrinchar hoje?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA CARF (do JS original) ***
    const placeholders = [
      'Resumo de ac�rd�os sobre planejamento tribut�rio?',
      'Quais c�maras julgam casos de IRPJ no CARF?',
      'Existe entendimento sobre multa qualificada?',
      'Como acompanhar o voto de qualidade ap�s a Lei 14302?'
    ];
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-scale-balanced'; // �cone do CARF
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('planejamento') || texto.includes('elis�o')) {
      return `O **CARF** vem distinguindo planejar tributos licitamente de simula��o.

**Elementos recorrentes em ac�rd�os:**
<ul>
  <li>Análise da subst�ncia econ�mica versus forma jur�dica</li>
  <li>Valoriza��o de prop�sitos negocial e operacional</li>
  <li>�nus probat�rio da fiscaliza��o para comprovar simula��o</li>
  <li>Impacto de pareceres e laudos independentes em defesa do contribuinte</li>
</ul>
Sugest�o: acompanhe as decis�es da 1� Turma da CSRF para teses recentes sobre reorganiza��es societ�rias.`;
    }

    if (texto.includes('multa qualificada') || texto.includes('150%')) {
      return `A **multa qualificada** exige dolo comprovado segundo precedentes do CARF.

**Pontos chaves:**
<ul>
  <li>Necessidade de demonstrar inten��o de fraude ou simula��o</li>
  <li>Debate sobre aplica��o autom�tica em casos de omiss�o de receita</li>
  <li>Redu��o da multa quando h� den�ncia espont�nea ou retifica��o tempestiva</li>
  <li>Diferen�as de entendimento entre c�maras superiores e turmas ordin�rias</li>
</ul>
Vale revisar ac�rd�os ap�s a Lei 14302 que modulam penalidades em empate pr� contribuinte.`;
    }

    if (texto.includes('voto de qualidade') || texto.includes('lei 14302') || texto.includes('desempate')) {
      return `A **Lei 14302/2022** converteu empates no CARF em decis�es favor�veis ao contribuinte.

**Efeitos observados:**
<ul>
  <li>Extin��o do voto de qualidade apenas para tributos federais</li>
  <li>Persist�ncia para processos aduaneiros e sancionat�rios</li>
  <li>Necessidade de registrar a ocorr�ncia de empate na ata</li>
  <li>Impacto retroativo limitado aos processos com julgamento ainda n�o conclu�do</li>
</ul>
Recomenda��o: acompanhe as portarias da PGFN sobre transa��es em casos com empate antes de 2022.`;
    }

    if (texto.includes('procedimento') || texto.includes('recurso') || texto.includes('prazo')) {
      return `No **contencioso do CARF**, atente-se aos prazos regimentais:

<ul>
  <li>Recurso volunt�rio: 30 dias ap�s ci�ncia da decis�o de primeira inst�ncia</li>
  <li>Manifesta��o de inconformidade: 30 dias para autos de infra��o</li>
  <li>Pedido de vistas: suspende prazo de 30 dias para vota��o</li>
  <li>Possibilidade de sustenta��o oral com inscri��o at� 48 horas antes da sess�o</li>
</ul>
Documenta��o completa e procura��es atualizadas evitam dilig�ncias adicionais.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Obrigado pela pergunta! Como especialista em **jurisprud�ncia do CARF**, estou pronto para apoio em:
<ul>
  <li>Pesquisa de ac�rd�os por c�mara, relator ou mat�ria</li>
  <li>Prepara��o de memoriais e notas t�cnicas</li>
  <li>Análise de riscos em autos de infra��o e multas</li>
  <li>Monitoramento de precedentes vinculantes</li>
  <li>Integração com estrat�gias cont�beis e fiscais</li>
</ul>
Envie mais detalhes sobre o caso que deseja estudar.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente CARF</strong><br><br>
