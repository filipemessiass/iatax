/**
 * ============================================
 * AGENT TAX CONSULT ICMS SE - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-se', 
    agentName: 'Agente Tax - Consultor ICMS - SE',
    apiContext: 'ICMS_SE',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - SE, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - SE:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente dedicado ao <strong>ICMS de Sergipe</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
        <li>Benefícios fiscais (PSDI), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substituição tributária, diferencial de alíquotas e operações interestaduais;</li>
        <li>Obrigações acessórias estaduais (EFD ICMS/IPI, DIC, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre o ICMS de Sergipe...',
      'Como funciona o PSDI (incentivo)?',
      'Qual a MVA da ST para autopeças em SE?',
      'Como funciona o FECOEP em Sergipe?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Sergipe (SE).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('psdi') || (texto.includes('benefício') && texto.includes('sergipe'))) {
      return `O **PSDI (Programa Sergipano de Desenvolvimento Industrial)** é o principal incentivo fiscal de Sergipe, focado em promover a industrialização.
      
      **O que oferece:**
      <ul>
        <li>Crédito presumido de ICMS (podendo chegar a 90% ou mais) sobre o saldo devedor do imposto.</li>
        <li>Diferimento (adiamento) do ICMS na importação de máquinas e equipamentos (ativo imobilizado).</li>
        <li>O percentual e as condições exatas dependem do tipo de indústria e do enquadramento no programa.</li>
      </ul>
      A adesão é formalizada através de um Termo de Acordo com o Governo do Estado.`;
    }

    if (texto.includes('st') || (texto.includes('substituição') && texto.includes('sergipe'))) {
      return `O **ICMS-ST em Sergipe** segue as regras gerais dos convênios CONFAZ, mas é importante notar:
      
      **Pontos de Atenção:**
      1.  **MVA/IVA-ST:** Sergipe utiliza suas próprias Margens de Valor Agregado (MVA), que estão detalhadas no Anexo IX do RICMS/SE.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para saídas interestaduais ou desfazimento do negócio) é regulamentado e exige a apresentação de demonstrativos específicos à SEFAZ-SE.
      3.  **Obrigações:** A **DIC (Declaração de Informações do Contribuinte)** é uma obrigação acessória estadual importante, além da EFD ICMS/IPI.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, Sergipe possui o **FECOEP (Fundo Estadual de Combate e Erradicação da Pobreza)**.
      
      **Como funciona:**
      <ul>
        <li>�? um adicional de **2%** sobre a alíquota do ICMS.</li>
        <li>Incide sobre produtos e serviços considerados "supérfluos", como bebidas alcoólicas, cigarros, perfumes, joias, energia elétrica acima de certa faixa, combustíveis, etc.</li>
        <li>A alíquota interna padrão em Sergipe é **19%**. Para esses produtos "supérfluos", a alíquota final se torna **21%** (19% + 2% FECOEP).</li>
        <li>O valor do FECOEP deve ser recolhido em guia específica.</li>
      </ul>`;
    }

    // Resposta Padrão (corrigida)
    return `Sou o especialista em <strong>ICMS de Sergipe</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ICMS - SE</strong><br><br>
