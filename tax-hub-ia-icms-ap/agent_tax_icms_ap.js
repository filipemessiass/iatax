/**
 * ============================================
 * AGENT TAX CONSULT ICMS AP - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ap', 
    agentName: 'Agente Tax - Consultor ICMS - AP',
    apiContext: 'ICMS_AP',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - AP, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - AP:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Amap�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (ALCA), incentivos regionais e programas de desenvolvimento;</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Amap�...',
      'Como funciona a ALCA (�rea de Livre Com�rcio)?',
      'Quais s�o os benef�cios fiscais do Amap�?',
      'Qual a al�quota interna de ICMS-AP?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Amap� (AP).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('alca') || (texto.includes('livre com�rcio') && texto.includes('amap�'))) {
      return `O Amap� possui a **ALCA (�rea de Livre Com�rcio de Macap� e Santana)**, que � um grande diferencial competitivo.
      
      **O que oferece:**
      <ul>
        <li>**Isen��o de ICMS:** Na entrada de produtos estrangeiros (importa��o) para comercializa��o ou industrializa��o dentro dos limites da ALCA.</li>
        <li>**Isen��o de ICMS:** Nas sa�das internas de produtos industrializados na ALCA (com insumos estrangeiros).</li>
        <li>**Cr�dito Est�mulo:** Para produtos industrializados no estado.</li>
      </ul>
      Para usufruir, a empresa precisa estar devidamente cadastrada e localizada nas �reas delimitadas (Macap� ou Santana) e cumprir os requisitos da legislação estadual.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('amap�'))) {
      return `O **ICMS-ST no Amap�** segue as regras gerais dos conv�nios CONFAZ, mas � importante notar:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O Amap� utiliza prioritariamente as MVAs (Margens de Valor Agregado) definidas nos conv�nios e protocolos. Na aus�ncia, aplica-se a MVA Ajustada.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado pelo RICMS/AP e exige procedimentos espec�ficos na EFD.
      3.  **Benef�cios da ALCA:** A ST *n�o* se aplica a muitas opera��es destinadas a empresas localizadas na ALCA, desde que os produtos sejam para comercializa��o ou industrializa��o dentro da �rea incentivada.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Amap� possui o **FECEP (Fundo Estadual de Combate e Erradica��o da Pobreza)**.
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A al�quota interna padr�o no Amap� � **18%**. Para esses produtos "sup�rfluos", a al�quota final se torna **20%** (18% + 2% FECEP).</li>
        <li>O valor do FECEP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Amap�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - AP</strong><br><br>
