/**
 * ============================================
 * AGENTE DOUTRINA IRPJ/CSLL - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-irpj',
    agentName: 'Agente Doutrinadores - IRPJ e CSLL',
    apiContext: 'DOUTRINA_IRPJ',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Reabra o agente quando quiser retomar os estudos de IRPJ e CSLL.'
  };

  // Lista de autores e obras (do JS original)
  const BIBLIOGRAFIA = [
    {
      autor: 'Fernanda Ogata e Thaianne Brito',
      obra: 'IRPJ e CSLL: Análise e Aplica��o',
      destaque: 'Guia pr�tico de apura��o pelo lucro real, com checklists e exemplos empresariais.'
    },
    {
      autor: 'Alexandre Alkmim Teixeira',
      obra: 'Tributação sobre a Renda: IRPJ/CSLL - Controv�rsias',
      destaque: 'Analisa temas complexos, inclusive planejamento tribut�rio e contencioso.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como Ogata organiza o checklist do lucro real trimestral?',
    'Quais controv�rsias Alexandre Alkmim destaca sobre juros sobre capital pr�prio?',
    'Melhor sequ�ncia de leitura para casos com preju�zo fiscal acumulado?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Ol�! Aqui voc� encontra refer�ncias essenciais de <strong>IRPJ e CSLL</strong>.
      <br><br>
      **Doutrinadores de refer�ncia:**
      <ul>
        ${listaAutores}
      </ul>
      Posso ajudar com checklists de apura��o, controv�rsias relevantes, cruzamento com julgados do CARF/STJ e dicas de compliance. Qual ponto de renda corporativa voc� precisa revisar hoje?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA DOUTRINA IRPJ ***
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-building-columns'; // �cone Doutrina IRPJ
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('ogata') || texto.includes('thaianne') || texto.includes('brito')) {
      return `A obra de **Fernanda Ogata e Thaianne Brito** organiza o lucro real em etapas: preparo da ECD, concilia��o de ajuste LALUR, monitoramento de saldo de preju�zo fiscal e reflexos na ECF. Cada cap�tulo traz fluxograma e quadro de decis�es para opera��es especiais.`;
    }

    if (texto.includes('alkmim') || texto.includes('alexandre') || texto.includes('controv�rsias') || texto.includes('planejamento')) {
      return `**Alexandre Alkmim Teixeira** aprofunda temas pol�micos como juros sobre capital pr�prio, subcapitaliza��o, tributa��o de lucros no exterior e impactos da Lei 12973. Sugere cruzar o material com decis�es recentes do CARF e do STJ para mapear riscos.`;
    }

    if (texto.includes('lucro presumido') || texto.includes('comparativo')) {
      return `Para comparar regimes, use os cap�tulos introdut�rios de Ogata e Brito sobre enquadramento e as tabelas de Alexandre Alkmim sobre margem de contribui��o. Monte planilha com base no setor e simule tr�s cen�rios (real, presumido e arbitrado).`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe o autor, a obra ou o tema espec�fico (ex: adi��es ao lucro real, jcp, preju�zo fiscal) e trago os trechos e boas pr�ticas relevantes das refer�ncias indicadas.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Doutrina IRPJ e CSLL</strong><br><br>
