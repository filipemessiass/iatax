/**
 * ============================================
 * AGENT TAX CONSULT ICMS GO - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-go', 
    agentName: 'Agente Tax - Consultor ICMS - GO',
    apiContext: 'ICMS_GO',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - GO, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - GO:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Goi�s</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (CTE-GO), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (PRODUZIR, FOMENTAR), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, DPI, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Goi�s...',
      'Como funciona o PRODUZIR?',
      'O que � o Fundo PROTEGE em Goi�s?',
      'Qual a al�quota interna de ICMS-GO?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Goi�s (GO).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('produzir') || texto.includes('fomentar') || (texto.includes('benef�cio') && texto.includes('goi�s'))) {
      return `Goi�s possui dois programas de incentivo muito conhecidos, o **PRODUZIR** e o **FOMENTAR**.
      
      **O que oferecem:**
      <ul>
        <li>Ambos s�o programas de **financiamento** do ICMS. A empresa paga o ICMS normal e o Estado "devolve" (financia) um percentual (ex: 70%) para ser pago em prazo longo e com juros baixos.</li>
        <li>O **PRODUZIR** � focado na implanta��o, expans�o ou relocaliza��o de ind�strias.</li>
        <li>O **FOMENTAR** � similar, mas tamb�m abrange o setor agroindustrial.</li>
      </ul>
      A ades�o � complexa e exige aprova��o de projeto pelo CD/FOMENTAR.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('goi�s'))) {
      return `O **ICMS-ST em Goi�s** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** Goi�s utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Ap�ndice II do Anexo VIII do RICMS/GO (CTE).
      ...
      3.  **DIFAL-ST:** Goi�s � muito rigoroso na cobran�a do ICMS-ST sobre o Diferencial de Al�quotas (DIFAL) nas compras interestaduais de ativo imobilizado ou uso/consumo, quando h� protocolo entre os estados.`;
    }

    if (texto.includes('protege') || texto.includes('fundo de pobreza')) {
      return `Sim, Goi�s possui o **PROTEGE GOI�S**, que � o Fundo de Prote��o Social do Estado de Goi�s.
      
      **Como funciona:**
      <ul>
        <li>� um adicional (geralmente de **2%**) sobre a al�quota do ICMS.</li>
        <li>Incide sobre produtos e servi�os considerados "sup�rfluos", como bebidas alco�licas, cigarros, perfumes, joias, energia el�trica, combust�veis, etc.</li>
        <li>A al�quota interna padr�o em Goi�s � **19%**. Para esses produtos "sup�rfluos", a al�quota final se torna **21%** (19% + 2% PROTEGE).</li>
        <li>O valor do PROTEGE deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Goi�s</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (CTE-GO), decretos, protocolos e conv�nios;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - GO</strong><br><br>
