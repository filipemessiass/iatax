/**
 * ============================================
 * AGENTE DOUTRINA ISS E RETEN��ES - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-iss-retencoes',
    agentName: 'Agente Doutrinadores - ISS e Retenções',
    apiContext: 'DOUTRINA_ISS_RETENCOES',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Reabra o agente quando precisar revisar ISS e reten��es em contratos.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Alexandre Marques',
    obra: 'Gest�o Tributária de Contratos e Conv�nios',
    destaque: 'Conte�do completo sobre reten��es na fonte (INSS, IRRF, CSLL, PIS/COFINS, ISS) com roteiros de auditoria.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Qual checklist a obra recomenda para reten��es em servi�os?',
    'Como tratar a reten��o de INSS em contratos com cooperativas?',
    'Quais s�o as principais armadilhas de ISS por local da presta��o?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Bem-vindo! Este agente re�ne orienta��es sobre <strong>ISS e reten��es na fonte</strong>.
      <br><br>
      **Refer�ncia principal:**
      <ul>
        <li><strong>${REFERENCIA.autor}</strong> (${REFERENCIA.obra}): ${REFERENCIA.destaque}</li>
      </ul>
      Auxilio na interpreta��o de cl�usulas contratuais, matriz de responsabiliza��o e cruzamentos com eSocial/DCTFWeb. Qual � sua d�vida atual?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    setInterval(() => {
      ...
      if (messageInput.value.trim() === '' && document.activeElement !== messageInput) {
        ...
        // Corrigindo encoding dos placeholders
        const placeholdersCorrigidos = [
          'Qual checklist a obra recomenda para reten��es em servi�os?',
          'Como tratar a reten��o de INSS em contratos com cooperativas?',
          'Quais s�o as principais armadilhas de ISS por local da presta��o?'
        ];
        messageInput.placeholder = `${placeholdersCorrigidos[placeholderIndex]} (Shift+Enter para nova linha)`;
      }
    }, 5200);
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA ISS/RETEN��ES ***
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-file-contract'; // �cone de ISS/Retenções
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('checklist') || texto.includes('contrato') || texto.includes('dossi�')) {
      return `**${REFERENCIA.autor}** sugere construir matriz de reten��es por tipo de servi�o, prevendo cl�usula contratual, munic�pio competente, eventos de eSocial e DCTFWeb. O livro traz modelos para anexar a cada contrato.`;
    }

    if (texto.includes('cooperativa') || texto.includes('inss') || texto.includes('prestador')) {
      return `A obra orienta verificar se h� cooperativa de trabalho, empresa individual ou prestador pessoa f�sica. Para cooperativas, o tomador recolhe 15% de INSS, al�m de verificar ISS no munic�pio sede. H� fluxograma espec�fico no cap�tulo 6.`;
    }

    if (texto.includes('local') || texto.includes('iss') || texto.includes('munic�pio')) {
      return `O livro apresenta tabela com os 31 servi�os do art. 3� da LC 116, destacando exce��es de local da presta��o e munic�pios com cadastro obrigat�rio. Utilize o quadro para evitar autua��es por fiscaliza��es cruzadas.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie o tema (INSS, ISS, IRRF, compliance contratual) e trarei os trechos aplic�veis do material de **${REFERENCIA.autor}**.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ISS e Retenções</strong><br><br>
