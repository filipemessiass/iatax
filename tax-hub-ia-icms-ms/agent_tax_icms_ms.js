/**
 * ============================================
 * AGENT TAX CONSULT ICMS MS - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ms', 
    agentName: 'Agente Tax - Consultor ICMS - MS',
    apiContext: 'ICMS_MS',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - MS, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - MS:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Mato Grosso do Sul</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (MS Empreendedor), incentivos regionais e programas de desenvolvimento;</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Mato Grosso do Sul...',
      'Como funciona o benef�cio "MS Empreendedor"?',
      'Como � o ICMS ST para gado no MS?',
      'Qual a al�quota interna de ICMS-MS?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Mato Grosso do Sul (MS).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('ms empreendedor') || (texto.includes('benef�cio') && texto.includes('ms'))) {
      return `Mato Grosso do Sul possui o **MS Empreendedor** (Decreto n� 15.340/2019), que � um dos principais incentivos do estado, focado em industrializa��o.
      
      **O que oferece:**
      <ul>
        <li>Concede um **Cr�dito Presumido** de ICMS, que pode chegar a **90%** do imposto devido nas sa�das, variando conforme o produto e o investimento.</li>
        <li>Diferimento (adiamento) do ICMS na importa��o de m�quinas (ativo imobilizado).</li>
      </ul>
      A ades�o � formalizada atrav�s de um Termo de Acordo com a SEMAGRO.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('ms'))) {
      return `O **ICMS-ST em Mato Grosso do Sul** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O MS utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Anexo III do RICMS/MS.
      ...
      3.  **Diferimento:** O MS possui muitas opera��es com diferimento, especialmente no setor do agroneg�cio (gado, soja, milho), que impactam a apura��o da ST na cadeia seguinte.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Mato Grosso do Sul possui o **FECOMP (Fundo Estadual de Combate e Erradica��o da Pobreza)**.
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A al�quota interna padr�o no MS � **17%**. Para esses produtos "sup�rfluos", a al�quota final se torna **19%** (17% + 2% FECOMP).</li>
        <li>O valor do FECOMP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Mato Grosso do Sul</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/MS), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (MS Empreendedor), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - MS</strong><br><br>
