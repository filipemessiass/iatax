/**
 * ============================================
 * AGENT TAX CONSULT ICMS RJ - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-rj', 
    agentName: 'Agente Tax - Consultor ICMS - RJ',
    apiContext: 'ICMS_RJ',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - RJ, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - RJ:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Rio de Janeiro</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/RJ), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (RIOLOG, RECOF), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA-ICMS, DECLAN-IPM, GNRE);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Rio de Janeiro...',
      'Como funciona o benef�cio RIOLOG?',
      'O que � o FECP no Rio de Janeiro?',
      'Qual a al�quota interna de ICMS-RJ?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Rio de Janeiro (RJ).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('riolog') || (texto.includes('benef�cio') && texto.includes('rj'))) {
      return `O Rio de Janeiro possui o **RIOLOG**, um regime especial para o setor atacadista e de log�stica (Decreto n� 44.498/2013).
      
      **O que oferece:**
      <ul>
        <li>**Diferimento (adiamento):** Do ICMS na importa��o de mercadorias para revenda.</li>
        <li>**Cr�dito Presumido:** Concede um cr�dito presumido na sa�da interestadual, resultando numa carga tribut�ria efetiva muito baixa (ex: 1,5%).</li>
        <li>**Condi��es:** Exige que a empresa opere como Centro de Distribui��o (CD) e cumpra requisitos de faturamento e localiza��o.</li>
      </ul>
      � um dos principais atrativos para empresas de com�rcio atacadista se instalarem no RJ.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('rj'))) {
      return `O **ICMS-ST no Rio de Janeiro** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O RJ utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Anexo I do Livro II do RICMS/RJ.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado e exige a entrega de arquivos e demonstrativos espec�ficos, sendo historicamente um processo complexo no estado.
      3.  **Obrigações:** A **GIA-ST** (Guia de Informa��o e Apura��o do ICMS-ST) � uma obriga��o acess�ria importante para contribuintes de outros estados que vendem com ST para o RJ.`;
    }

    if (texto.includes('fecp') || texto.includes('fundo de pobreza')) {
      return `Sim, o Rio de Janeiro possui o **FECP (Fundo Estadual de Combate � Pobreza)**, e ele � muito relevante.
      
      **Como funciona:**
      <ul>
        <li>� um adicional de **2%** sobre a al�quota do ICMS.</li>
        <li>**IMPORTANTE:** Diferente de outros estados, o FECP no RJ incide sobre **quase todas as opera��es**, n�o apenas sobre sup�rfluos.</li>
        <li>A al�quota interna padr�o no RJ � **18%**. Com o FECP, a al�quota pr�tica para a maioria dos produtos � **20%** (18% + 2%).</li>
        <li>Produtos sup�rfluos (como cigarros, bebidas, etc.) t�m al�quotas ainda maiores, que tamb�m recebem o adicional do FECP.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Rio de Janeiro</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/RJ), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (RIOLOG, RECOF), incentivos regionais e programas de desenvolvimento;</li>
        <li>C�lculo e regras do **FECP** (Fundo de Pobreza);</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA-ICMS, DECLAN-IPM, GNRE).</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - RJ</strong><br><br>
