/**
 * ============================================
 * AGENT TAX CONSULT ICMS TO - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-to', 
    agentName: 'Agente Tax - Consultor ICMS - TO',
    apiContext: 'ICMS_TO',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - TO, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - TO:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Tocantins</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (PROINDUSTRIA), incentivos regionais e programas de desenvolvimento;</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Tocantins...',
      'Como funciona o PROINDUSTRIA?',
      'Qual a al�quota interna de ICMS-TO?',
      'Como funciona o FECOEP em Tocantins?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Tocantins (TO).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('proindustria') || (texto.includes('benef�cio') && texto.includes('tocantins'))) {
      return `Tocantins possui o **PROINDUSTRIA** (Programa de Industrializa��o, Emprego e Renda), que � um dos principais incentivos fiscais do estado.
      
      **O que oferece:**
      <ul>
        <li>Concede um **Cr�dito Presumido** de ICMS, que pode chegar a **75%** sobre o saldo devedor do imposto apurado mensalmente.</li>
        <li>Diferimento (adiamento) do ICMS na importa��o de m�quinas (ativo imobilizado) e na aquisi��o de energia el�trica.</li>
      </ul>
      A ades�o � formalizada atrav�s de um Termo de Acordo de Regime Especial (TARE) com a SEFAZ-TO.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('tocantins'))) {
      return `O **ICMS-ST em Tocantins** segue as regras gerais dos conv�nios CONFAZ.
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** Tocantins utiliza as MVAs (Margens de Valor Agregado) definidas nos conv�nios e protocolos. Na aus�ncia, aplica-se a MVA Ajustada.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado pelo RICMS/TO e exige procedimentos espec�ficos.
      3.  **Antecipa��o Parcial:** Tocantins exige o recolhimento da "Antecipa��o Parcial" do ICMS nas entradas interestaduais de mercadorias para revenda, calculada pela diferen�a de al�quotas.`;
    }

    if (texto.includes('al�quota') || texto.includes('aliquota interna') || texto.includes('fecop')) {
      return `A **Al�quota Interna** padr�o do ICMS em Tocantins � de **20%** (a partir de 2024).
      
      **Fundo de Pobreza (FECOP-TO):**
      Tocantins tamb�m possui o FECOEP, que adiciona **2%** � al�quota de produtos e servi�os "sup�rfluos" (como bebidas alco�licas, cigarros, perfumes, joias, energia el�trica, combust�veis, etc.), elevando a carga m�xima para **22%** (20% + 2%).`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Tocantins</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - TO</strong><br><br>
