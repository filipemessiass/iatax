/**
 * ============================================
 * AGENT TAX CONSULT ICMS PR - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-pr', 
    agentName: 'Agente Tax - Consultor ICMS - PR',
    apiContext: 'ICMS_PR',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - PR, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - PR:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Paran�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/PR), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (Paran� Competitivo), incentivos regionais e programas de desenvolvimento;</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Paran�...',
      'Como funciona o "Paran� Competitivo"?',
      'O que � o FECOP no Paran�?',
      'Qual a al�quota interna de ICMS-PR?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Paran� (PR).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('paran� competitivo') || (texto.includes('benef�cio') && texto.includes('paran�'))) {
      return `O Paran� possui o **Paran� Competitivo**, um dos programas de incentivo mais robustos do estado para atra��o de investimentos.
      
      **O que oferece:**
      <ul>
        <li>**Diferimento (adiamento):** Do ICMS na importa��o e aquisi��o interna de ativo imobilizado (m�quinas).</li>
        <li>**Cr�dito Presumido:** Concede um cr�dito presumido sobre o saldo devedor do ICMS, reduzindo a carga tribut�ria efetiva.</li>
        <li>**Suspens�o/Diferimento:** Em opera��es com insumos e mat�ria-prima.</li>
      </ul>
      A ades�o � complexa, exige um protocolo de inten��es e assinatura de Termo de Acordo com o governo do estado.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('paran�'))) {
      return `O **ICMS-ST no Paran�** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O Paran� utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Anexo IX do RICMS/PR.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado e exige a entrega de arquivos e demonstrativos espec�ficos (ADRC-ST).
      3.  **GIA-ST:** Contribuintes de outros estados que vendem com ST para o Paran� devem entregar a GIA-ST.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Paran� possui o **FECOP (Fundo Estadual de Combate � Pobreza)**.
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A al�quota interna padr�o no Paran� � **19%**. Para esses produtos "sup�rfluos", a al�quota final se torna **21%** (19% + 2% FECOP).</li>
        <li>O valor do FECOP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Paran�</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/PR), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (Paran� Competitivo), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA-ST, ADRC-ST);</li>
        <li>Procedimentos de fiscaliza��o, concilia��o de documentos fiscais e regimes especiais.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - PR</strong><br><br>
