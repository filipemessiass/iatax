/**
 * ============================================
 * AGENT TAX CONSULT ICMS MT - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-mt', 
    agentName: 'Agente Tax - Consultor ICMS - MT',
    apiContext: 'ICMS_MT',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - MT, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - MT:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Mato Grosso</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (PRODEIC), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA-ST, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Mato Grosso...',
      'Como funciona o PRODEIC?',
      'O que � o FETHAB?',
      'Qual a al�quota interna de ICMS-MT?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Mato Grosso (MT).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('prodeic') || (texto.includes('benef�cio') && texto.includes('mato grosso'))) {
      return `Mato Grosso possui o **PRODEIC (Programa de Desenvolvimento Industrial e Comercial)**, que � o principal incentivo fiscal do estado.
      
      **O que oferece:**
      <ul>
        <li>Concede um **Cr�dito Presumido** de ICMS, que pode chegar a 85% do imposto devido nas sa�das, variando conforme o produto.</li>
        <li>Em alguns casos, oferece redu��o da base de c�lculo.</li>
        <li>O objetivo � atrair ind�strias e centros de distribui��o (CDs) para o estado.</li>
      </ul>
      A ades�o � formalizada atrav�s de um Termo de Acordo com a SEDEC/MT.`;
    }

    if (texto.includes('fethab')) {
      return `O **FETHAB (Fundo Estadual de Transporte e Habita��o)** � uma contribui��o espec�fica do Mato Grosso, muito importante para o agroneg�cio.
      
      **Como funciona:**
      <ul>
        <li>Incide sobre opera��es com soja, gado, algod�o, madeira, etc.</li>
        <li>N�o � exatamente um ICMS, mas uma **contribui��o** devida por produtores rurais e frigor�ficos, calculada por tonelada ou por cabe�a (no caso do gado).</li>
        <li>O n�o pagamento do FETHAB pode levar � suspens�o de benef�cios fiscais do ICMS, por isso � crucial estar em dia.</li>
      </ul>`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('mato grosso'))) {
      return `O **ICMS-ST em Mato Grosso** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O MT utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Ap�ndice X do RICMS/MT.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST � regulamentado e exige a entrega de arquivos e demonstrativos espec�ficos.
      3.  **Antecipa��o (Estimativa):** O MT utiliza muito o regime de "Estimativa por Opera��o" ou "Estimativa Simplificada (Carga M�dia)", onde o imposto � exigido na entrada da mercadoria no estado, com base em pautas ou percentuais fixos, substituindo o regime normal de apura��o.`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Mato Grosso</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/MT), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (PRODEIC), incentivos regionais e programas de desenvolvimento;</li>
        <li>Contribuições como o **FETHAB**;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e o regime de Estimativa;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - MT</strong><br><br>
