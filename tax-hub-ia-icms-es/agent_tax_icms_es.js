/**
 * ============================================
 * AGENT TAX CONSULT ICMS ES - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-es', 
    agentName: 'Agente Tax - Consultor ICMS - ES',
    apiContext: 'ICMS_ES',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - ES, selecione novamente o agente.'
  };
  ...
  function notificarFechamento() {
    ...
    document.dispatchEvent(new CustomEvent('chat:closed', { detail: detalalhe }));
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - ES:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Esp�rito Santo</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/ES), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (INVEST-ES, COMPETE-ES), incentivos regionais e programas;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, DOT, GNRE, entre outras);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Esp�rito Santo...',
      'Como funciona o INVEST-ES?',
      'O que � o COMPETE-ES para e-commerce?',
      'Qual a al�quota interna de ICMS-ES?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Esp�rito Santo (ES).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('invest') || (texto.includes('benef�cio') && texto.includes('esp�rito santo'))) {
      return `O Esp�rito Santo � muito conhecido pelo **INVEST-ES (Programa de Incentivo ao Investimento no Estado do Esp�rito Santo)**.
      
      **O que oferece:**
      <ul>
        <li>**Diferimento (adiamento):** Do ICMS na importa��o de m�quinas (ativo imobilizado) e insumos.</li>
        <li>**Cr�dito Presumido:** Concede um cr�dito presumido (geralmente de 70% a 90%) sobre o saldo devedor do ICMS nas sa�das.</li>
        <li>**Redu��o de Base:** Pode incluir redu��o da base de c�lculo em opera��es espec�ficas.</li>
      </ul>
      � um incentivo muito forte para ind�strias e Centros de Distribui��o, mas exige um projeto detalhado aprovado pelo estado.`;
    }

    if (texto.includes('compete') || texto.includes('e-commerce')) {
      return `O **COMPETE-ES** � um benef�cio fiscal do Esp�rito Santo focado em atacadistas e, especialmente, em **opera��es de e-commerce (n�o presenciais)**.
      
      **Como funciona (E-commerce):**
      <ul>
        <li>A empresa apura o ICMS normal (d�bito - cr�dito).</li>
        <li>Sobre o **valor do imposto a pagar** nas sa�das interestaduais para consumidor final, a empresa recebe um **estorno de d�bito** (um tipo de cr�dito) que reduz a carga tribut�ria efetiva para pr�ximo de **1%**.</li>
        <li>� um dos benef�cios mais agressivos do Brasil para e-commerce.</li>
      </ul>`;
    }

    if (texto.includes('al�quota') || texto.includes('aliquota interna') || texto.includes('fundo de pobreza')) {
      return `A **Al�quota Interna** padr�o do ICMS no Esp�rito Santo � de **17%**.
      
      **Fundo de Pobreza (FECP-ES):**
      O ES possui o Fundo Estadual de Combate � Pobreza, que adiciona **2%** � al�quota de produtos e servi�os "sup�rfluos" (como bebidas alco�licas, cigarros, perfumes, joias, energia el�trica, combust�veis, etc.), elevando a carga m�xima para **19%** (17% + 2%).`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Esp�rito Santo</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS/ES), decretos, protocolos e conv�nios;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - ES</strong><br><br>
