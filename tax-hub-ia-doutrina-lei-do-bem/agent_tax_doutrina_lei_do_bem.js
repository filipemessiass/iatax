/**
 * ============================================
 * AGENTE DOUTRINA LEI DO BEM - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-lei-do-bem',
    agentName: 'Agente Doutrinadores - Lei do Bem',
    apiContext: 'DOUTRINA_LEI_DO_BEM',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte quando quiser revisar rotinas da Lei do Bem e seus incentivos.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Christimara Garcia',
    obra: 'Lei do Bem: Como Alavancar a Inovação com a Utiliza��o dos Incentivos Fiscais',
    destaque: 'Guia pr�tico que detalha elegibilidade, c�lculo de benef�cios e governan�a documental.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Quais s�o os requisitos b�sicos de elegibilidade na Lei do Bem?',
    'Como montar o dossi� t�cnico e cont�bil para auditoria?',
    'Quais cruzamentos fiscais a autora recomenda com ECF e SPED?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Sou o agente focado na <strong>Lei do Bem</strong>, ideal para empresas que investem em P&D.
      <br><br>
      **Refer�ncia principal:**
      <ul>
        <li><strong>${REFERENCIA.autor}</strong> (${REFERENCIA.obra}): ${REFERENCIA.destaque}</li>
      </ul>
      Posso ajudar com requisitos de elegibilidade, montagem do dossi�, cruzamento com ECF e di�logo com o MCTI. Qual aspecto voc� deseja revisar agora?`;
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-flask'; // �cone da Lei do Bem
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('crit�rio') || texto.includes('criterio') || texto.includes('elegibilidade')) {
      return `**${REFERENCIA.autor}** destaca que os projetos devem envolver inovação de produto ou processo, possuir relat�rios t�cnicos, contabilidade segregada e alinhamento com o Manual Frascati. Os cap�tulos iniciais explicam como validar o enquadramento junto ao MCTI.`;
    }

    if (texto.includes('documento') || texto.includes('dossi�') || texto.includes('compliance')) {
      return `A autora sugere montar um dossi� com: plano de projeto, memoriais de c�lculo, atas do comit� de inovação, parecer t�cnico do MCTI e concilia��o com a ECF. H� um checklist no final do livro que pode ser adaptado ao cronograma anual.`;
    }

    if (texto.includes('c�lculo') || texto.includes('benef�cio') || texto.includes('dedu��o')) {
      return `Para calcular a dedu��o adicional, utilize a planilha proposta por **${REFERENCIA.autor}**, que separa despesas de P&D por natureza, aplica os multiplicadores legais (20%, 60%, 80%) e gera o espelho para a ECF. H� exemplos de integração com incentivos estaduais.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie uma d�vida sobre enquadramento, documentos, c�lculo de dedu��o ou integração com projetos de inovação, e apresento os trechos relevantes da obra de **${REFERENCIA.autor}**.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Lei do Bem</strong><br><br>
