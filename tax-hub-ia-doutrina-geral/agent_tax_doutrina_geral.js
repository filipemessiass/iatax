/**
 * ============================================
 * AGENTE DOUTRINA GERAL - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-geral',
    agentName: 'Agente Doutrinadores - Doutrina Geral',
    apiContext: 'DOUTRINA_GERAL',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Sempre que precisar retomar os manuais de doutrina geral, reabra este agente.'
  };

  // Lista de autores e obras utilizados nas respostas (do JS original)
  const BIBLIOGRAFIA = [
    {
      autor: 'Leandro Paulsen',
      obra: 'Curso de Direito Tribut�rio Completo',
      destaque: 'Integra jurisprud�ncia do STF e do STJ com abordagem sistem�tica.'
    },
    {
      autor: 'Hugo de Brito Machado',
      obra: 'Curso de Direito Tribut�rio',
      destaque: 'Cl�ssico da teoria geral com forte fundamenta��o conceitual.'
    },
    {
      autor: 'Roque Antonio Carraza',
      obra: 'Curso de Direito Constitucional Tribut�rio',
      destaque: 'Refer�ncia sobre princ�pios constitucionais e compet�ncia tribut�ria.'
    },
    {
      autor: 'Ricardo Alexandre',
      obra: 'Direito Tribut�rio',
      destaque: 'Manual atualizado com �nfase na reforma tribut�ria e em simulados.'
    },
    {
      autor: 'Eduardo Sabbag',
      obra: 'Manual de Direito Tribut�rio',
      destaque: 'Did�tico, com linguagem direta e recursos visuais para concursos.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Quais cap�tulos do Paulsen tratam de processos administrativos?',
    'Resumo r�pido da abordagem do Carraza sobre princ�pio da legalidade?',
    'Comparativo entre Machado e Sabbag para estudo de concursos?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Ol�! Sou o agente voltado aos **manuais gerais de direito tribut�rio**.
      <br><br>
      **Doutrinadores de refer�ncia:**
      <ul>
        ${listaAutores}
      </ul>
      Posso ajudar voc� a identificar cap�tulos-chave, comparativos entre abordagens e relacionar cada doutrinador com decis�es recentes. Como posso apoiar seu estudo agora?`;
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-book'; // �cone de Doutrina
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    for (const item of BIBLIOGRAFIA) {
      const sobrenome = item.autor.split(' ').pop().toLowerCase();
      const nome = item.autor.split(' ')[0].toLowerCase();
      if (texto.includes(sobrenome) || texto.includes(nome)) {
        return `**${item.autor}** (${item.obra}):\n\n*${item.destaque}* \n\nSugest�o: consulte os cap�tulos iniciais para consolidar fundamentos e, em seguida, revise os resumos ao final de cada unidade.`;
      }
    }

    if (texto.includes('comparativo') || texto.includes('concurso')) {
      return `Para estudos simult�neos, utilize **Ricardo Alexandre** e **Eduardo Sabbag** para mapas mentais e simulados, e recorra a **Hugo de Brito Machado** para fundamenta��o te�rica. Combine revis�es semanais com quest�es comentadas.`;
    }

    if (texto.includes('princ�pio') || texto.includes('constitucional') || texto.includes('compet�ncia')) {
      return `Aprofunde-se com **Roque Antonio Carraza**, que relaciona princ�pios constitucionais, reparti��o de compet�ncias e controle pelo STF. Use tamb�m a obra de **Leandro Paulsen** para cruzar com precedentes recentes.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Posso ajudar com resumos, sugest�es de ordem de leitura, conex�es com jurisprud�ncia ou dicas de estudo.\n\nEnvie o nome do autor, obra ou princ�pio que deseja explorar e trago os trechos mais relevantes.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Doutrina Geral</strong><br><br>
