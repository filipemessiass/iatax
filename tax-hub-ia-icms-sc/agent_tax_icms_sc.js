/**
 * ============================================
 * AGENT TAX CONSULT ICMS SC - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-sc', 
    agentName: 'Agente Tax - Consultor ICMS - SC',
    apiContext: 'ICMS_SC',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - SC, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - SC:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Santa Catarina</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS-SC), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (TTD - Tratamento Tribut�rio Diferenciado), incentivos regionais (Pr�-Emprego);</li>
        <li>Substitui��o tribut�ria (ST), diferencial de al�quotas (DIFAL) e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, DIME, DCIP, GNRE);</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Santa Catarina...',
      'Como funciona o TTD 409 (Importa��o)?',
      'O que � o Pr�-Emprego?',
      'Qual a al�quota interna de ICMS-SC?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Santa Catarina (SC).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('ttd') || (texto.includes('benef�cio') && texto.includes('santa catarina'))) {
      return `Santa Catarina � famosa pelos **TTDs (Tratamentos Tribut�rios Diferenciados)**. N�o � um programa �nico, mas sim um conjunto de regimes especiais concedidos caso a caso.
      
      **Benef�cios Comuns (TTDs):**
      <ul>
        <li>**TTD 409/410/411 (Importa��o):** Permite o diferimento (adiamento) do ICMS na importa��o de mercadorias para revenda, e concede um cr�dito presumido na sa�da interestadual, reduzindo a carga efetiva.</li>
        <li>**Cr�dito Presumido:** Diversos setores (t�xtil, alimentos, metal-mec�nico) possuem TTDs que concedem cr�dito presumido em vez do cr�dito normal (d�bito/cr�dito).</li>
        <li>**Pr�-Emprego:** Um incentivo focado na gera��o de empregos, que pode incluir diferimento de ICMS sobre ativo imobilizado.</li>
      </ul>
      A ades�o a um TTD exige um processo administrativo na SEF-SC.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('sc'))) {
      return `O **ICMS-ST em Santa Catarina** segue as regras gerais dos conv�nios CONFAZ, mas possui particularidades:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** Santa Catarina utiliza suas pr�prias MVAs (Margens de Valor Agregado), que est�o definidas no Anexo 3 do RICMS-SC.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado e exige a entrega da **DRCST** (Demonstrativo para Recupera��o, Ressarcimento ou Complementa��o do ICMS-ST).
      3.  **GIA-ST:** Contribuintes de outros estados que vendem com ST para SC devem entregar a GIA-ST.`;
    }

    if (texto.includes('al�quota') || texto.includes('aliquota interna') || texto.includes('fundo de pobreza')) {
      return `A **Al�quota Interna** padr�o do ICMS em Santa Catarina � de **17%**.
      
      **Al�quotas Espec�ficas:**
      <ul>
        <li>**12%:** Para diversos produtos (ex: energia el�trica, combust�veis, transporte).</li>
        <li>**25%:** Para produtos considerados "sup�rfluos" (ex: bebidas alco�licas, cigarros, perfumes, joias).</li>
      </ul>
      
      **Fundo de Pobreza (FECEP):**
      Santa Catarina **n�o possui** um Fundo de Combate � Pobreza adicionado ao ICMS, diferentemente da maioria dos outros estados. Isso torna a al�quota interna (17%) mais competitiva.`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Santa Catarina</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual (RICMS-SC), decretos, protocolos e conv�nios;</li>
        <li>Benef�cios fiscais (**TTDs**), incentivos regionais (Pr�-Emprego);</li>
        <li>Substitui��o tribut�ria (ST), diferencial de al�quotas (DIFAL) e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, DIME, DCIP, GNRE);</li>
        <li>Procedimentos de fiscaliza��o e regimes especiais.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - SC</strong><br><br>
