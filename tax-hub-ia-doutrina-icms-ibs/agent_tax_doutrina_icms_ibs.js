/**
 * ============================================
 * AGENTE DOUTRINA ICMS E IBS - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-icms-ibs',
    agentName: 'Agente Doutrinadores - ICMS e IBS',
    apiContext: 'DOUTRINA_ICMS_IBS',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte quando quiser revisar ICMS, IBS e seus manuais de refer�ncia.'
  };
  ...
  const BIBLIOGRAFIA = [
    {
      ...
      obra: 'ICMS',
      destaque: 'Obra cl�ssica, atualizada para a transição ao IBS e fundamentada em princ�pios constitucionais.'
    },
    {
      autor: 'Cl�udio Borba', // Corrigido encoding
      obra: 'ICMS',
      destaque: 'Abordagem did�tica com exerc�cios e foco em concursos da �rea fiscal.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como Carraza analisa a adapta��o do ICMS ao IBS?',
    'Quais cap�tulos de Cl�udio Borba refor�am a pr�tica de cr�ditos?',
    'Sugest�es para estudos de compet�ncia e guerra fiscal?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Sauda��es! Este agente re�ne refer�ncias para a <strong>transição do ICMS ao IBS</strong>.
      <br><br>
      **Obras recomendadas:**
      <ul>
        ${listaAutores}
      </ul>
      Ajudo a conectar fundamentos constitucionais, simulados e casos pr�ticos. Qual aspecto voc� deseja explorar: guerra fiscal, cr�ditos, transição ou conformidade estadual?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA ICMS/IBS ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-map' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-map'; // �cone do ICMS/IBS
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('carraza') || texto.includes('roque')) {
      return `**Roque Antonio Carraza** analisa a transição com foco nos princ�pios constitucionais, limite de compet�ncia dos estados e modula��o da futura LC. Use seus cap�tulos finais para entender fundos de compensa��o e distribui��o federativa.`;
    }

    if (texto.includes('borba') || texto.includes('exerc�cio') || texto.includes('concurso')) {
      return `**Cl�udio Borba** organiza o ICMS com resumo por t�pico e listas de quest�es, excelente para refor�ar c�lculo de cr�ditos, diferimento e substitui��o tribut�ria. Adapte os exerc�cios para simular cen�rios com IBS durante a fase de conviv�ncia.`;
    }

    if (texto.includes('guerra fiscal') || texto.includes('benef�cio') || texto.includes('convalida��o')) {
      return `Combine a leitura constitucional de Carraza com os quadros pr�ticos de Borba para mapear benef�cios convalidados e cl�usulas de transição. Atualize seu banco de dados com os decretos do Conselho Federativo e legislação estadual complementar.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe o autor, tema ou d�vida (cr�ditos, guerra fiscal, IBS) e trarei os trechos relevantes das obras de Carraza e Borba.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS e IBS</strong><br><br>
