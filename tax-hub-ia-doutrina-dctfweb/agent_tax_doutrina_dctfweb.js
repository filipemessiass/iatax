/**
 * ============================================
 * AGENTE DOUTRINA DCTFWEB - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-dctfweb',
    agentName: 'Agente Doutrinadores - DCTFWeb',
    apiContext: 'DOUTRINA_DCTFWEB',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Retorne quando quiser revisar rotinas da DCTFWeb e seus cruzamentos.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Fred Amaral e Jacian Anisio Marques da Silva',
    obra: 'Manuais pr�ticos e artigos sobre DCTFWeb',
    destaque: 'Re�nem a experi�ncia de um CEO de tax tech e de um auditor-fiscal gestor do projeto, com foco em opera��o e malhas.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como os autores orientam o fechamento mensal da DCTFWeb?',
    'Quais cruzamentos com eSocial e EFD Reinf devem ser verificados?',
    'Como tratar notifica��es de diferen�a emitidas pela Receita?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Use este agente para dominar a <strong>DCTFWeb</strong> com apoio de especialistas.
      <br><br>
      **Refer�ncia principal:**
      <ul>
        <li><strong>${REFERENCIA.autor}</strong> (${REFERENCIA.obra}): ${REFERENCIA.destaque}</li>
      </ul>
      Auxilio mitigar diverg�ncias, integrar com eSocial/eReinf e preparar malhas. Qual etapa voc� deseja analisar?`;
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-network-wired'; // �cone da DCTFWeb
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('fechamento') || texto.includes('cronograma')) {
      return `**${REFERENCIA.autor}** sugerem roteiro: validar eventos de eSocial e Reinf, conferir totais no eCAC, gerar DCTFWeb, emitir DARF num�rico e agendar pagamentos. H� lista de verifica��es para cada dia �til do calend�rio.`;
    }

    if (texto.includes('cruzamento') || texto.includes('erros') || texto.includes('malha')) {
      return `Os autores destacam cruzamentos autom�ticos entre eSocial, Reinf e DCTFWeb, orientando a usar relat�rios de inconsist�ncias e a ferramenta de auditoria para identificar diferen�as de base ou contribuintes sem recolhimento.`;
    }

    if (texto.includes('notifica��o') || texto.includes('diferen�a') || texto.includes('recolhimento')) {
      return `O material indica como acessar notifica��es na caixa postal, gerar DARF complementar, protocolar defesa e acompanhar parcelamentos. H� fluxograma para atua��es preventivas junto � PGFN.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie sua d�vida sobre rotina (fechamento, concilia��o, malhas ou defesas) e trago orienta��es baseadas na experi�ncia de **${REFERENCIA.autor}**.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente DCTFWeb</strong><br><br>
