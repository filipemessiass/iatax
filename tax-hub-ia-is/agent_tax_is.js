/**
 * ============================================
 * AGENT TAX CONSULT IS (IMPOSTO SELETIVO) - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'is',
    agentName: 'Agente Tax - Consultor IS',
    apiContext: 'IS',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre o Imposto Seletivo, selecione novamente o agente.'
  };
   ...
   * O conte�do foi retirado do 'agent_tax_is.html' original.
   */
  function adicionarMensagemInicial() {
    // Corrigimos a codifica��o (ex: Ol&aacute; -> Ol�!)
    const mensagemInicial = `Ol�! Sou o consultor para o <strong>Imposto Seletivo</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Identifica��o de produtos e servi�os sujeitos ao IS;</li>
        <li>C�lculo de bases, al�quotas e cr�ditos vinculados;</li>
        <li>Controles de estoque e ajuste de precifica��o;</li>
        <li>Integração com IBS, CBS e demais obriga��es acess�rias;</li>
        <li>Monitoramento de regulamenta��es e conformidade operacional.</li>
      </ul>
      Qual d�vida voc� quer esclarecer?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA IMPOSTO SELETIVO (IS) ***
    const placeholders = [
      'Descreva sua d�vida sobre o Imposto Seletivo...',
      'Quais produtos pagam o Imposto Seletivo?',
      'O IS gera cr�dito para o IBS/CBS?',
      'Qual a al�quota do IS?',
      'Como funciona a extra��o de min�rio?'
    ];
  ...
  function enviarMensagem() {
    ...
    // Usamos um 'setTimeout' para simular o tempo de D
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA IMPOSTO SELETIVO (IS) ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-industry' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-industry'; // �cone do IS
    avatarDiv.appendChild(avatarIcon);
   ...
   * uma resposta pr�-definida baseada nos t�picos do IS.
   *
  ...
  function gerarRespostaBot(mensagem) {
    ...
    // T�pico 1: Identifica��o de Produtos
    if (texto.includes('produto') || texto.includes('quais') || texto.includes('lista') || texto.includes('incide')) {
      return `O **Imposto Seletivo (IS)** incidir� sobre a produ��o, comercializa��o ou importa��o de bens e servi�os considerados prejudiciais � sa�de ou ao meio ambiente.
      
      **Exemplos principais (previstos na Reforma):**
      <ul>
        <li>Ve�culos automotores (crit�rio de polui��o);</li>
        <li>Embarca��es e aeronaves (exceto uso comercial);</li>
        <li>Produtos fum�genos (cigarros, etc.);</li>
        <li>Bebidas alco�licas (cerveja, vinho, destilados);</li>
        <li>Bebidas a�ucaradas (refrigerantes, sucos industrializados);</li>
        <li>Extra��o de min�rio de ferro, petr�leo e g�s natural (para exporta��o).</li>
      </ul>
      A lista final e as al�quotas exatas depender�o da Lei Complementar.`;
    }

    // T�pico 2: C�lculo e Cr�ditos
    if (texto.includes('cr�dito') || texto.includes('credito') || texto.includes('cbs') || texto.includes('ibs')) {
      return `Este � um ponto crucial: O **Imposto Seletivo (IS) N�O GERA CR�DITO**.
      
      Diferente do IBS e da CBS (que s�o n�o cumulativos e geram cr�ditos em cada etapa), o IS tem car�ter "extra-fiscal" (para desestimular o consumo):
      <ul>
        <li>Ele ser� cobrado **uma �nica vez** (monof�sico) na cadeia (ex: na ind�stria ou importa��o).</li>
        <li>O valor do IS pago **n�o poder�** ser usado para abater o IBS ou a CBS.</li>
        <li>Ele ser� um custo que compor� o pre�o final do produto.</li>
      </ul>`;
    }

    // T�pico 3: Al�quotas e Base
    if (texto.includes('al�quota') || texto.includes('aliquota') || texto.includes('base') || texto.includes('c�lculo')) {
      return `As al�quotas do **Imposto Seletivo (IS)** ainda ser�o definidas por Lei Complementar e variar�o para cada produto.
      
      **O que sabemos:**
      <ul>
        <li>O IS poder� ter al�quotas **espec�ficas** (um valor fixo por unidade, ex: R$ 0,50 por ma�o de cigarro) ou **"ad valorem"** (um percentual sobre o valor da opera��o).</li>
        <li>A al�quota m�xima para extra��o mineral ser� de **1%**.</li>
        <li>Para os demais itens (cigarros, bebidas, carros), as al�quotas ser�o definidas para desestimular o consumo, mas ainda n�o foram publicadas.</li>
      </ul>`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Sou o consultor para o <strong>Imposto Seletivo</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Identifica��o de produtos e servi�os sujeitos ao IS;</li>
        <li>C�lculo de bases, al�quotas e se ele gera cr�ditos;</li>
        <li>Impacto na precifica��o e controles de estoque;</li>
        <li>Integração com IBS e CBS.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Imposto Seletivo (IS)</strong><br><br>
