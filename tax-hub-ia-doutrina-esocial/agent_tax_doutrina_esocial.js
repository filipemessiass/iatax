/**
 * ============================================
 * AGENTE DOUTRINA ESOCIAL - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-esocial',
    agentName: 'Agente Doutrinadores - eSocial',
    apiContext: 'DOUTRINA_ESOCIAL',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte quando quiser revisar os eventos e rotinas do eSocial.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Zenaide Carvalho',
    obra: 'eSocial nos �rg�os p�blicos',
    destaque: 'Considerada autoridade pr�tica no tema, apresenta passo a passo de implanta��o, matriz de responsabilidades e rotinas de auditoria.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como Zenaide estrutura a governan�a do eSocial?',
    'Quais eventos cr�ticos ela recomenda monitorar diariamente?',
    'Sugest�es para tratar n�o conformidades identificadas pelo RET?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Este agente usa a experi�ncia de <strong>${REFERENCIA.autor}</strong> para orientar sua jornada no eSocial.
      <br><br>
      **Refer�ncia principal:**
      <ul>
        <li><strong>${REFERENCIA.autor}</strong> (${REFERENCIA.obra}): ${REFERENCIA.destaque}</li>
      </ul>
      Auxilio com implanta��o, rotina di�ria, cruzamentos com DCTFWeb e RET. Qual evento ou processo voc� deseja revisar?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-user-check' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-user-check'; // �cone do eSocial
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('governan�a') || texto.includes('processo') || texto.includes('papel')) {
      return `**${REFERENCIA.autor}** recomenda criar comit� eSocial, definir pap�is (RH, TI, fiscal, jur�dico), mapear fontes de dados e implementar revis�es peri�dicas dos layouts.`;
    }

    if (texto.includes('evento') || texto.includes('s-1200') || texto.includes('s-2299')) {
      return `O livro destaca eventos cr�ticos: S-1200 (remunera��o), S-1210 (pagamento), S-2200 (admiss�o) e S-2299 (desligamento). Fornece checklists para validar antes da transmiss�o e orienta usar o RET para diagnosticar erros.`;
    }

    if (texto.includes('ret') || texto.includes('n�o conformidade') || texto.includes('auditoria')) {
      return `Zenaide sugere monitorar o RET e o painel de pend�ncias, guardar logs de envio e criar plano de corre��o com prazos e respons�veis.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe sua d�vida (eventos, cronograma, RET, integração com DCTFWeb) e trago os trechos adequados da obra de **${REFERENCIA.autor}**.`;
  }
