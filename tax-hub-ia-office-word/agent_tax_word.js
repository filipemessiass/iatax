...
(() => {
  ...
  const CONFIG = {
    agentId: 'word',
    agentName: 'Agente Tax - Consultor Word',
    apiContext: 'PACOTE_OFFICE_WORD',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Quando precisar revisar documentos novamente, volte a abrir o agente Word.'
  };
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Boas-vindas! Eu sou o **especialista em Word** do TaxHub IA.
      <br><br>
      Posso orientar voc� em:
      <ul>
        <li>Cria��o de estilos padr�o e aplica��o de normas (ABNT, corporativas)</li>
        <li>Sum�rios e �ndices automatizados com campo atualizado</li>
        <li>Mala direta para correspond�ncias fiscais e comerciais</li>
        <li>Controle de altera��es, compara��o de vers�es e revis�o colaborativa</li>
        <li>Vistas protegidas, modelos .dotx e bibliotecas de conte�do reutiliz�vel</li>
      </ul>
      Qual aspecto da documenta��o voc� deseja aprimorar agora?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA WORD (do JS original) ***
    const placeholders = [
      'Como montar um modelo padr�o com estilos?',
      'Qual a melhor forma de criar um sum�rio autom�tico?',
      'Como usar mala direta para cartas personalizadas?',
      'De que modo o controle de altera��es deve ser aplicado?'
    ];
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA WORD ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-file-word' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-file-word'; // �cone do Word
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('estilo') || texto.includes('formata��o') || texto.includes('modelo')) {
      return `**Estilos padr�o** s�o a base da padroniza��o documental.

**Passo a passo:**
1. Defina um modelo \`.dotx\` com estilos personalizados (T�tulo 1, Subt�tulo, Corpo do Texto).
2. Use *Selecionar texto com formato similar* para aplicar estilos rapidamente.
3. Bloqueie ajustes diretos (Bot�o direito > Modificar > Desmarcar "Atualizar automaticamente").
4. Armazene o modelo em uma biblioteca do SharePoint para garantir distribui��o centralizada.

Com estilos bem definidos, sum�rios, �ndices e refer�ncias cruzadas permanecem coerentes.`;
    }

    if (texto.includes('sum�rio') || texto.includes('indice')) {
      return `Para **sum�rio autom�tico**:

1. Aplique estilos de t�tulo nos cap�tulos (T�tulo 1, T�tulo 2, etc.).
2. Posicione o cursor onde o sum�rio deve aparecer e acesse *Refer�ncias > Sum�rio*.
3. Escolha um modelo autom�tico ou personalizado.
4. Atualize o campo com \`F9\` sempre que editar t�tulos.

Para permitir navega��o r�pida, ative o painel de navega��o (Exibir > Painel de Navega��o).`;
    }

    if (texto.includes('mala direta') || texto.includes('mail merge') || texto.includes('carta')) {
      return `A **mala direta** agiliza correspond�ncias personalizadas:

1. Inicie em *Correspond�ncias > Iniciar Mala Direta* e escolha o tipo (Carta, Email, Etiquetas).
2. Selecione a fonte de dados: planilha Excel, lista do Outlook ou SharePoint.
3. Insira campos mesclados (Nome, Raz�o Social, Vencimento) onde desejar.
4. Antes de concluir, use *Visualizar Resultados* para checar os campos.
5. Finalize em *Concluir e Mesclar*, escolhendo gerar documento �nico ou envio direto.`;
    }

    if (texto.includes('revis�o') || texto.includes('coment�rio') || texto.includes('controle de altera��es')) {
      return `Para **revis�o colaborativa**:

1. Ative *Revis�o > Controle de Altera��es* antes de editar.
2. Use *Novo Comentário* para explicar ajustes ou solicitar revis�o.
3. Configure *Painel de Revis�o* para visualizar todas as altera��es pendentes.
4. Quando finalizar, utilize *Aceitar Tudo* ou *Rejeitar Tudo* conforme a decis�o.

Em ambientes corporativos, combine com a fun��o *Restri��o de Edi��o* para delimitar quem pode alterar cada se��o.`;
    }

    if (texto.includes('refer�ncia cruzada') || texto.includes('hiperlink') || texto.includes('legenda')) {
      return `**Refer�ncias cruzadas** mant�m figuras e anexos atualizados:

1. Insira legendas com *Refer�ncias > Inserir Legenda* (Figura 1, Tabela 2, etc.).
2. Posicione o cursor no texto e selecione *Refer�ncias > Refer�ncia Cruzada*.
3. Escolha o tipo (Figura, Tabela, T�tulo) e defina se quer texto ou n�mero da p�gina.
4. Ao reorganizar o documento, atualize com \`Ctrl+A\` seguido de \`F9\`.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Agrade�o o contato! Como especialista em **Word profissional**, posso apoiar voc� em:
<ul>
  <li>Padronizar manuais, contratos e pareceres</li>
  <li>Gerenciar sum�rios, �ndices, legendas e notas de rodap�</li>
  <li>Configurar mala direta e etiquetas personalizadas</li>
  <li>Controlar vers�es, revis�es e prote��es de documento</li>
  <li>Automatizar blocos de texto com Galeria de Componentes R�pidos</li>
</ul>
Conte mais detalhes sobre o tipo de documento e elaboro um passo a passo sob medida.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Word</strong><br><br>
