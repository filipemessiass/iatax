/**
 * ============================================
 * AGENTE DOUTRINA PIS/COFINS E CBS - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-pis-cbs',
    agentName: 'Agente Doutrinadores - PIS/COFINS e CBS',
    apiContext: 'DOUTRINA_PIS_CBS',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Retorne quando quiser revisar a transição para CBS e IBS.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Mary Elbe Queiroz e Antonio Carlos de Souza Junior',
    obra: 'IBS e CBS: Análise aprofundada do novo modelo de tributa��o do consumo',
    destaque: 'Detalha a transição do PIS/COFINS para CBS, com reflexos no IBS e nas obriga��es acess�rias.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Quais s�o os prazos de transição do PIS/COFINS para CBS segundo Mary Elbe?',
    'Como ficam os cr�ditos acumulados na virada para CBS?',
    'Qual rela��o a obra faz entre CBS e IBS no Conselho Federativo?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Sou o agente dedicado ao <strong>PIS/COFINS na transição para CBS</strong>.
      <br><br>
      **Refer�ncia principal:**
      <ul>
        <li><strong>${REFERENCIA.autor}</strong> (${REFERENCIA.obra}): ${REFERENCIA.destaque}</li>
      </ul>
      Posso orientar sobre cr�ditos, prazos, ajustes de sistemas e integração com o IBS. Qual ponto voc� deseja analisar?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA PIS/CBS ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-layer-group' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-layer-group'; // �cone do PIS/CBS
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('prazo') || texto.includes('transição')) {
      return `**${REFERENCIA.autor}** destacam que h� per�odo de conv�vio between PIS/COFINS e CBS, com redu��o gradativa das al�quotas antigas e entrada da CBS conforme cronograma (ainda pendente de LC). Sugere-se monitorar o Conselho Federativo para ajustes trimestrais.`;
    }

    if (texto.includes('cr�dito') || texto.includes('saldo')) {
      return `A obra detalha o aproveitamento de cr�ditos acumulados por meio de compensa��o, ressarcimento ou convers�o em certificados para abatimento de CBS. Recomenda-se reconciliar saldos na ECF e registrar ajustes na transição para evitar glosas.`;
    }

    if (texto.includes('ibs') || texto.includes('governan�a') || texto.includes('conselho')) {
      return `Os autores explicam como o Conselho Federativo coordenar� o IBS e a CBS, com redistribui��o de receitas entre estados e munic�pios. H� cap�tulos dedicados ao comit� gestor e ao regime de coopera��o fiscal.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie uma d�vida sobre cr�ditos, prazos, governan�a ou integração com sistemas, e apresento os pontos relevantes da obra de **${REFERENCIA.autor}**.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente PIS/COFINS e CBS</strong><br><br>
