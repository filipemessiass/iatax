/**
 * ============================================
 * AGENT TAX CONSULT ICMS PB - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-pb', 
    agentName: 'Agente Tax - Consultor ICMS - PB',
    apiContext: 'ICMS_PB',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - PB, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - PB:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente dedicado ao <strong>ICMS da Paraíba</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre o ICMS da Paraíba...',
      'O que é o FAIN (Fundo de Apoio)?',
      'Como funciona o FECOEP da Paraíba?',
      'Qual a alíquota interna padrão de ICMS-PB?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Paraíba (PB).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('fain') || (texto.includes('benefício') && texto.includes('paraíba'))) {
      return `O **FAIN (Fundo de Apoio ao Desenvolvimento Industrial da Paraíba)** é o principal programa de incentivo do estado, regulamentado pela Lei nº 7.653/2004.
      
      **O que oferece:**
      <ul>
        <li>Concede um **Crédito Presumido** de ICMS sobre o saldo devedor apurado.</li>
        <li>O percentual do crédito pode variar (ex: 75%, 90%) dependendo do tipo de indústria, localização e número de empregos gerados.</li>
        <li>Frequentemente, exige um percentual de "retorno" (pagamento) ao próprio fundo.</li>
      </ul>
      A adesão depende da aprovação de um projeto de viabilidade pelo Conselho Deliberativo do FAIN.`;
    }

    if (texto.includes('st') || (texto.includes('substituição') && texto.includes('paraíba'))) {
      return `O **ICMS-ST na Paraíba** segue as regras gerais dos convênios CONFAZ, mas possui particularidades:
      
      **Pontos de Atenção:**
      1.  **MVA/IVA-ST:** A Paraíba utiliza suas próprias Margens de Valor Agregado (MVA), que são definidas no Regulamento do ICMS (RICMS-PB).
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para saídas interestaduais ou vendas a consumidores finais abaixo da base presumida) é detalhado e exige arquivos magnéticos específicos (CAT-17).
      3.  **Fronteiras:** O estado possui postos fiscais ativos e o controle de mercadorias em trânsito é rigoroso, exigindo o pagamento antecipado em muitos casos de entrada.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, a Paraíba possui o **FECOP (Fundo Estadual de Combate e Erradicação da Pobreza)**.
      
      **Como funciona:**
      <ul>
        <li>�? um adicional de **2%** sobre a alíquota do ICMS.</li>
        <li>Incide sobre produtos e serviços considerados "supérfluos", como bebidas alcoólicas, cigarros, perfumes, joias, energia elétrica acima de 100 kWh, etc.</li>
        <li>A alíquota interna padrão da Paraíba é **18%**. Para esses produtos "supérfluos", a alíquota final se torna **20%** (18% + 2% FECOEP).</li>
        <li>O valor do FECOEP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padrão (corrigida)
    return `Sou o especialista em <strong>ICMS da Paraíba</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e convênios do estado;</li>
        <li>Benefícios fiscais (FAIN), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substituição tributária, diferencial de alíquotas e operações interestaduais;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ICMS - PB</strong><br><br>
