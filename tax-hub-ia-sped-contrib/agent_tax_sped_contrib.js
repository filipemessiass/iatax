/**
 * ============================================
 * AGENT TAX CONSULT SPED CONTRIBUI��ES - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'sped-contrib',
    agentName: 'Agente Tax - Consultor SPED Contribuicoes',
    apiContext: 'SPED_CONTRIB',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre SPED Contribuicoes, selecione novamente o agente.'
  };
   ...
   * O conte�do foi retirado do 'agent_tax_sped_contrib.html' original.
   */
  function adicionarMensagemInicial() {
    // Corrigimos a codifica��o (ex: Ola!)
    const mensagemInicial = `Ol�! Vamos trabalhar no <strong>SPED Contribuições</strong>.
      <br><br>
      Posso orientar sobre:
      <ul>
        <li>Estrutura dos blocos 0, C, D, F e registros espec�ficos;</li>
        <li>Apura��o de cr�ditos e d�bitos de PIS/COFINS nos regimes cumulativo e n�o cumulativo;</li>
        <li>Tratamento de receitas financeiras, exclus�es e ajustes de Base de C�lculo;</li>
        <li>Rela��o com notas fiscais eletr�nicas, EFD ICMS/IPI e PER/DCOMP;</li>
        <li>Boas pr�ticas de compliance e controle de riscos tribut�rios.</li>
      </ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    // *** CONTE�DO ADAPTADO PARA SPED CONTRIBUI��ES ***
    const placeholders = [
      'Descreva sua d�vida sobre SPED Contribuições...',
      'O que � o Bloco C (Receitas)?',
      'Como apurar cr�ditos no Bloco M?',
      'Diferen�a de regime cumulativo e n�o cumulativo?',
      'O que � o Bloco F (Outras Opera��es)?'
    ];
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA SPED CONTRIBUI��ES ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-file-lines' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-file-lines'; // �cone do EFD Contribuições
    avatarDiv.appendChild(avatarIcon);
   ...
   * uma resposta pr�-definida baseada nos t�picos do EFD Contribuições.
   *
  ...
  function gerarRespostaBot(mensagem) {
    ...
    // T�pico 1: Cr�ditos / N�o Cumulativo / Bloco M
    if (texto.includes('cr�dito') || texto.includes('credito') || (texto.includes('bloco') && texto.includes('m')) || texto.includes('n�o cumulativo')) {
      return `O regime **N�o Cumulativo** de PIS/COFINS permite a tomada de cr�ditos sobre diversos custos e despesas (insumos, alugu�is, energia, etc.).
      
      **Na EFD Contribuições:**
      <ul>
        <li>Os cr�ditos s�o detalhados nos Blocos C (NF-e), D (Servi�os) e F (Outras Opera��es, ex: Alugu�is).</li>
        <li>O **Bloco M** � o "cora��o" da apura��o. � onde os cr�ditos (M100/M500) e os d�bitos (M200/M600) s�o consolidados para gerar o valor a pagar.</li>
        <li>� fundamental que o CST (C�digo da Situa��o Tributária) esteja correto na nota fiscal de entrada para permitir o cr�dito.</li>
      </ul>`;
    }

    // T�pico 2: Cumulativo
    if (texto.includes('cumulativo') || texto.includes('presumido')) {
      return `O regime **Cumulativo** (geralmente usado por empresas do Lucro Presumido) � mais simples:
      
      <ul>
        <li>**N�o h� tomada de cr�ditos.** O PIS/COFINS � calculado diretamente sobre a receita bruta.</li>
        <li>Na EFD Contribuições, a apura��o � feita no Bloco M (Registros M200/M600), mas os campos de cr�dito (M100/M500) n�o s�o preenchidos.</li>
        <li>As receitas s�o detalhadas nos Blocos A (Servi�os) e C (Mercadorias), com CSTs espec�ficos de regime cumulativo.</li>
      </ul>`;
    }

    // T�pico 3: Blocos C, D, F
    if (texto.includes('bloco c') || texto.includes('bloco d') || texto.includes('bloco f')) {
      return `Os blocos C, D e F s�o usados para detalhar as opera��es que geram d�bitos ou cr�ditos:
      
      <ul>
        <li>**Bloco C:** Detalha documentos fiscais de **mercadorias** (NF-e, NFC-e). Usado tanto para receitas (d�bitos) quanto para aquisi��es (cr�ditos).</li>
        <li>**Bloco D:** Detalha documentos de **servi�os** (ex: Notas de Servi�o de Comunica��o ou Transporte).</li>
        <li>**Bloco F:** Usado para **Outras Opera��es e Ajustes**. � muito comum para registrar cr�ditos sobre despesas que n�o t�m documento fiscal pr�prio (ex: alugu�is - F100), receitas financeiras (F500), ou ajustes de apura��o.</li>
      </ul>`;
    }
    
    // T�pico 4: Receitas Financeiras / Ajustes
    if (texto.includes('financeira') || texto.includes('ajuste') || texto.includes('exclus�o')) {
      return `O tratamento de Receitas Financeiras e Ajustes � um ponto cr�tico:
      
      <ul>
        <li>**Receitas Financeiras (Regime N�o Cumulativo):** S�o tributadas a 0,65% (PIS) e 4% (COFINS), mesmo para empresas do N�o Cumulativo. Devem ser informadas no Bloco F (ex: F500 ou F550).</li>
        <li>**Exclus�es da Base de C�lculo:** Vendas canceladas, devolu��es e descontos incondicionais s�o exclus�es comuns que devem ser registradas no Bloco M (M205/M605) para reduzir o d�bito a pagar.</li>
      </ul>`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Ol�! Vamos trabalhar no <strong>SPED Contribuições</strong>.
      <br><br>
      Posso orientar sobre:
      <ul>
        <li>Estrutura dos blocos 0, C, D, F e registros espec�ficos;</li>
        <li>Apura��o de cr�ditos e d�bitos (regimes cumulativo e n�o cumulativo);</li>
        <li>Tratamento de receitas financeiras e exclus�es;</li>
        <li>Rela��o com NF-e e PER/DCOMP.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente SPED Contribuições</strong><br><br>
