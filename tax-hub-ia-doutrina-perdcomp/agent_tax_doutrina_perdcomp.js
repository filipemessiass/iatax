/**
 * ============================================
 * AGENTE DOUTRINA PER/DCOMP - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-perdcomp',
    agentName: 'Agente Doutrinadores - PER/DCOMP',
    apiContext: 'DOUTRINA_PERDCOMP',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Reabra o agente sempre que precisar revisar PER/DCOMP e cr�ditos fiscais.'
  };

  // Lista de autores e obras (do JS original)
  const REFERENCIA = {
    autor: 'Luciana Leite e Jeni Carla Fritzke Schulter',
    obra: 'Manuais pr�ticos e e-books sobre PER/DCOMP',
    destaque: 'Apresentam estudos de caso, orientação passo a passo e cuidados para evitar glosas nos pedidos.'
  };

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Quais documentos devo reunir antes de enviar um PER/DCOMP?',
    'Como registrar um pedido de ressarcimento de PIS/Cofins?',
    'Quais erros comuns geram glosa segundo as autoras?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Vamos organizar os <strong>pedidos PER/DCOMP</strong> com base nas obras de <strong>${REFERENCIA.autor}</strong>.
      <br><br>
      **Refer�ncia:**
      <ul>
        <li>${REFERENCIA.obra}. ${REFERENCIA.destaque}</li>
      </ul>
      Estou pronto para ajudar na constru��o do dossi�, valida��o de cr�ditos e envio de arquivos. Qual ponto deseja revisar?`;
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('documento') || texto.includes('dossi�') || texto.includes('comprovante')) {
      return `**${REFERENCIA.autor}** sugerem reunir laudos de cr�dito, EFDs, comprovantes de pagamento, memoriais de c�lculo e evid�ncias cont�beis. Os materiais incluem modelos de planilhas para anexar ao pedido.`;
    }

    if (texto.includes('ressarcimento') || texto.includes('pis') || texto.includes('cofins')) {
      return `As autoras demonstram passo a passo no PER/DCOMP Web, destacando c�digos de receita, transmiss�o e acompanhamento do status. Recomenda-se conciliar o saldo com a EFD Contribuições e a ECF.`;
    }

    if (texto.includes('glosa') || texto.includes('erro') || texto.includes('fiscaliza��o')) {
      return `O material lista erros comuns como diverg�ncia de per�odo, falta de documenta��o e uso indevido de cr�dito. Apresenta estrat�gia de defesa administrativa e atualiza��o do dossi�.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe a natureza do cr�dito, tipo de pedido ou etapa do PER/DCOMP e apresento orienta��es espec�ficas baseadas nos materiais de **${REFERENCIA.autor}**.`;
  }
