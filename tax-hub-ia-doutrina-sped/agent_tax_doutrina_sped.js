/**
 * ============================================
 * AGENTE DOUTRINA SPED - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-sped',
    agentName: 'Agente Doutrinadores - SPED',
    apiContext: 'DOUTRINA_SPED',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte quando precisar rever SPED, ECD ou ECF com apoio dos doutrinadores.'
  };

  // Lista de autores e obras (do JS original)
  const BIBLIOGRAFIA = [
    {
      autor: 'Fellipe Guerra',
      obra: 'Descomplicando o SPED',
      destaque: 'Explica o ecossistema completo com foco em educa��o, parametriza��o e concilia��o cont�bil.'
    },
    {
      autor: 'Mauro Negruni',
      obra: 'Compreendendo a ECF',
      destaque: 'Pioneiro do SPED, enfatiza integração de dados e governan�a de informa��es fiscais.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como Fellipe Guerra sugere estruturar o time de SPED?',
    'Quais cruzamentos Mauro Negruni recomenda entre ECF e EFD Contribuições?',
    'Checklist para auditoria digital antes do envio da ECF?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Ol�! Vamos descomplicar o <strong>SPED e suas declara��es</strong>.
      <br><br>
      <ul>
        ${listaAutores}
      </ul>
      Trago orienta��es sobre integração de sistemas, concilia��o de saldos, auditoria digital e preven��o de autua��es. Qual arquivo voc� quer revisar?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA DOUTRINA SPED ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-database' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-database'; // �cone Doutrina SPED
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('fellipe') || texto.includes('guerra')) {
      return `**Fellipe Guerra** sugere estruturar o time de SPED em c�lulas (cont�bil, fiscal, tecnologia) e criar painel de monitoramento de eventos. O livro traz planilhas de controle por tipo de arquivo e cronogramas de fechamento.`;
    }

    if (texto.includes('negruni') || texto.includes('ecf')) {
      return `**Mauro Negruni** detalha a integração da ECF com ECD, EFD Contribuições e EFD ICMS/IPI. Destaca auditorias automatizadas e o uso de trilhas de auditoria para validar saldos e lucros presumidos.`;
    }

    if (texto.includes('auditoria') || texto.includes('malha') || texto.includes('confer�ncia')) {
      return `Combine o checklist de Fellipe (rotinas de valida��o) com os scripts de Negruni para cruzar Saldos, plano de contas referencial, e-blocos, invent�rio e eSocial.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe o arquivo (ECD, ECF, eFinanceira, Reinf) ou autor e apresento resumos, checklists e refer�ncias para sua necessidade.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente SPED</strong><br><br>
