/**
 * ============================================
 * AGENT TAX CONSULT ISS - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'iss',
    agentName: 'Agente Tax - Consultor ISS',
    apiContext: 'ISS',
    // AJUSTE DE TEXTO: Corrigido
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ISS, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ISS:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Olá! Eu sou o seu assistente para questões de <strong>ISS</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Enquadramento de serviços na lista da LC 116/2003 e legislações locais;</li>
        <li>Definição de município competente, substituição e responsabilidade tributária;</li>
        <li>Regimes especiais, retenções e obrigações acessórias municipais;</li>
        <li>Tratamento de estabelecimentos prestadores/tomadores e retenção na fonte;</li>
        <li>Integração com NFSe, DAPI, DES-IF e demais declarações.</li>
      </ul>
      Em que tema de ISS você precisa de apoio?`;
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua dúvida sobre ISS...',
      'Qual o local de recolhimento do ISS?',
      'Quando devo reter o ISS na fonte?',
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-building-columns';
    avatarDiv.appendChild(avatarIcon);
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ISS (LC 116, Local, Retenção).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('lc 116') || texto.includes('lista de serviços')) {
      return `A **Lei Complementar 116/2003** é a lei nacional do ISS. Ela define:
      
      <ul>
        <li>A **Lista de Serviços** anexa, que determina quais atividades são tributáveis pelo ISS.</li>
        <li>As regras gerais de **local de recolhimento** (onde o imposto é devido).</li>
        <li>As alíquotas mínimas (2%) e máximas (5%).</li>
      </ul>
      �? fundamental verificar o código do serviço (ex: "1.01", "17.05") na lista para definir a tributação.`;
    }

    if (texto.includes('local') || texto.includes('onde pagar') || texto.includes('recolhimento')) {
      return `Essa é a dúvida mais comum sobre ISS! A **regra geral** (Art. 3º da LC 116) é que o imposto é devido no município do **estabelecimento prestador**.
      
      Porém, existem **25 exceções** importantes (como construção civil, limpeza, segurança, feiras) onde o imposto é devido no **local da execução do serviço** (onde o tomador está).
      
      �? crucial analisar o tipo de serviço (da Lista) para saber qual regra aplicar e evitar pagar o imposto no município errado.`;
    }

    if (texto.includes('reter') || texto.includes('retenção na fonte')) {
      return `A **retenção do ISS na fonte** ocorre quando a lei municipal obriga o **tomador** (quem contrata o serviço) a reter e pagar o imposto, em vez do prestador.
      
      Isso é comum quando:
      <ul>
        <li>O prestador é de outro município e o serviço é uma das exceções (ex: limpeza, construção civil).</li>
        <li>O tomador é um "Substituto Tributário" (ex: órgãos públicos, grandes empresas) designado pela prefeitura.</li>
      </ul>
      A falta de retenção, quando obrigatória, pode gerar multa para o tomador.`;
    }

    if (texto.includes('nfse') || texto.includes('nota fiscal')) {
      return `A **NFSe (Nota Fiscal de Serviços Eletrônica)** é o documento digital que substituiu as antigas notas de papel. 
      
      Cada prefeitura tem seu próprio sistema (ou usa um provedor nacional). Recentemente, o Governo Federal criou o **Padrão Nacional da NFSe**, que visa unificar a emissão para todos os municípios, facilitando a vida das empresas que prestam serviços em várias cidades.`;
    }

    // Resposta Padrão (corrigida)
    return `Sou especialista em <strong>ISS</strong> e posso ajudar com:
      <ul>
        <li>Enquadramento de serviços na lista da LC 116/2003 e legislações locais;</li>
        <li>Definição de município competente, substituição e responsabilidade tributária;</li>
        <li>Regimes especiais, retenções e obrigações acessórias municipais;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>�s�️ Erro ao iniciar o agente ISS</strong><br><br>
