/**
 * ============================================
 * AGENTE DOUTRINA TRABALHISTA/PREVIDENCIARIA - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-trabalhista',
    agentName: 'Agente Doutrinadores - Tributação Trabalhista',
    apiContext: 'DOUTRINA_TRABALHISTA',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte sempre que precisar revisar tributa��o trabalhista e previdenci�ria.'
  };
  ...
  const BIBLIOGRAFIA = [
    {
      ...
      obra: 'Tributação e Riscos do Trabalho',
      destaque: 'Ex-presidente do CARF, oferece vis�o pr�tica de fiscaliza��es e defesa administrativa.'
    },
    {
      autor: 'Halley Henares Neto e Pedro Teixeira Leite Ackel',
      obra: 'Tributação Trabalhista e Previdenciária',
      destaque: 'Especialistas em contencioso e consultoria, com casos reais e roteiros de compliance.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Qual abordagem Carlos Henrique usa para fiscaliza��es de folha?',
    'Como Halley Henares recomenda documentar PLR?',
    'Sugest�es de leitura sobre risco trabalhista em terceiriza��o?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Ol�! Vamos aprofundar a <strong>tributa��o trabalhista e previdenci�ria</strong> com refer�ncias de tribunal e consultoria.
      <br><br>
      <ul>
        ${listaAutores}
      </ul>
      Posso apoiar na estrutura de defesas, avalia��o de riscos e adequa��o ao eSocial/DCTFWeb. Qual d�vida voc� quer esclarecer?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA TRABALHISTA ***
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-briefcase'; // �cone Trabalhista
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    if (texto.includes('carlos') || texto.includes('oliveira') || texto.includes('carf')) {
      return `**Carlos Henrique de Oliveira** enfatiza controles internos, revis�o de rubricas de folha, impacto de PLR, stock options e fiscaliza��o conjunta da Receita. A obra traz checklists para auditorias de conformidade e estrat�gias de defesa no CARF.`;
    }

    if (texto.includes('halley') || texto.includes('ackel') || texto.includes('contencioso')) {
      return `**Halley Henares Neto** e **Pedro Ackel** apresentam casos reais de contencioso previdenci�rio, incluindo reenquadramento de verbas, responsabiliza��o de tomadores e gest�o de passivos. Utilizam matriz de risco por centro de custo.`;
    }

    if (texto.includes('terceiriza��o') || texto.includes('risco') || texto.includes('auditoria')) {
      return `Combine as orienta��es de Oliveira (sobre provas e contratos) com os fluxos de Henares/Ackel para monitorar terceiros, validar documentos e registrar evid�ncias no eSocial.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Informe o autor, tema ou verba (PLR, horas extras, pens�es, terceiros) e trago os trechos aplic�veis das obras recomendadas.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Tributação Trabalhista</strong><br><br>
