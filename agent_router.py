...
Roteia consultas para o agente mais adequado
"""
...
class AgentRouter:
    ...
    def route(self, query: str, context: dict = None) -> RoutingResult:
        """Roteia consulta para agente mais adequado."""
        ...
        # Se só tem 1 agente ativo, rota pra ele
        if len(active_agents) == 1:
            return RoutingResult(
                primary_agent=active_agents[0],
                confidence=1.0,
                reasoning=f"Único agente ativo: {active_agents[0]}"
            )
        ...
        # Se score muito baixo, usar fallback
        if best_score < 0.1:
            return RoutingResult(
                primary_agent=active_agents[0],
                confidence=0.5,
                reasoning=f"Fallback para {active_agents[0]} (score baixo)"
            )
        
        return RoutingResult(
            primary_agent=best_agent,
            confidence=min(best_score, 1.0),
            reasoning=f"Match por palavras-chave: {best_score:.2f}"
        )
    
    def _calculate_keyword_score(self, query: str, keywords: List[str]) -> float:
        """Calcula score de match entre query e keywords."""
        if not keywords:
            return 0.0
    ...
    def get_routing_suggestions(self, query: str) -> List[Tuple[str, float]]:
        """Retorna top 3 sugestões de roteamento."""
        query_lower = query.lower()
        active_agents = self.registry.get_all_agent_ids()
