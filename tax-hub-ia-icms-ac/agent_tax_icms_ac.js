/**
 * ============================================
 * AGENT TAX CONSULT ICMS AC - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-ac', 
    agentName: 'Agente Tax - Consultor ICMS - AC',
    apiContext: 'ICMS_AC',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - AC, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - AC:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS do Acre</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS do Acre...',
      'Quais s�o os benef�cios fiscais do Acre?',
      'O que � a ZPE Acre?',
      'Como funciona o FECOP no Acre?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Acre (AC).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('zpe') || (texto.includes('benef�cio') && texto.includes('acre'))) {
      return `O Acre possui um grande diferencial que � a **ZPE (Zona de Processamento de Exporta��o)**, localizada em Senador Guiomard.
      
      **O que �?**
      � uma �rea de livre com�rcio com o exterior, destinada � instala��o de empresas voltadas para a produ��o de bens a serem exportados.
      
      **Benef�cios Fiscais (ICMS):**
      <ul>
        <li>**Suspens�o do ICMS:** Na importa��o ou aquisi��o interna de m�quinas, equipamentos e insumos pela empresa autorizada a operar na ZPE.</li>
        <li>**Isen��o nas Sa�das:** As exporta��es de produtos da ZPE para o exterior s�o imunes ao ICMS.</li>
      </ul>
      Este � um dos regimes mais fortes do estado para empresas com foco em exporta��o.`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('acre'))) {
      return `O **ICMS-ST no Acre** segue as regras gerais dos conv�nios CONFAZ, mas � importante notar:
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** O Acre utiliza as MVAs (Margens de Valor Agregado) definidas nos conv�nios e protocolos. Na aus�ncia, aplica-se a MVA Ajustada com base na al�quota interna.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST (para sa�das interestaduais) � regulamentado pelo RICMS/AC e exige a entrega de arquivos e demonstrativos espec�ficos.
      3.  **Antecipa��o:** O Acre exige o ICMS Antecipado na entrada de mercadorias no estado, com regras espec�ficas de c�lculo e recolhimento.`;
    }

    if (texto.includes('fecop') || texto.includes('fundo de pobreza')) {
      return `Sim, o Acre possui o **FECOP (Fundo Estadual de Combate e Erradica��o da Pobreza)**.
      ...
      **Como funciona:**
      <ul>
        ...
        <li>A al�quota interna padr�o no Acre � **19%**. Para esses produtos "sup�rfluos", a al�quota final se torna **21%** (19% + 2% FECOP).</li>
        <li>O valor do FECOP deve ser recolhido em guia separada.</li>
      </ul>`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS do Acre</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (incluindo a **ZPE**), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
        <li>Obrigações acess�rias estaduais (EFD ICMS/IPI, GIA, GNRE, entre outras);</li>
        <li>Procedimentos de fiscaliza��o, concilia��o de documentos fiscais e regimes especiais.</li>
      </ul>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - AC</strong><br><br>
