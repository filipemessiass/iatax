/**
 * ============================================
 * AGENT TAX CONSULT ICMS RR - SCRIPT PRINCIPAL
...
(() => {
  ...
  const CONFIG = {
    agentId: 'icms-rr', 
    agentName: 'Agente Tax - Consultor ICMS - RR',
    apiContext: 'ICMS_RR',
    // AJUSTE DE TEXTO: Corrigido e padronizado
    closingMessage: 'Conversa encerrada. Para um novo atendimento sobre ICMS - RR, selecione novamente o agente.'
  };
  ...
  function iniciarChat() {
    ...
    try {
      ...
      console.error('Erro ao inicializar o agente ICMS - RR:', erro);
      mostrarErroNoChat();
    }
  }
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagemInicial = `Ol�! Eu sou o seu assistente dedicado ao <strong>ICMS de Roraima</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (ALC), incentivos regionais e programas de desenvolvimento;</li>
  ...
  function adicionarListenersCustomizados() {
    ...
    const placeholders = [
      'Descreva sua d�vida sobre o ICMS de Roraima...',
      'Como funciona a ALC de Boa Vista e Bonfim?',
      'Qual a al�quota interna de ICMS-RR?',
      'Como funciona o FECOEP em Roraima?'
    ];
   ...
   * AJUSTE DE TEXTO: Respostas criadas para ICMS Roraima (RR).
   */
  function gerarRespostaBot(mensagem) {
    const texto = mensagem.toLowerCase();

    if (texto.includes('alc') || (texto.includes('livre com�rcio') && texto.includes('roraima'))) {
      return `Roraima possui a **�rea de Livre Com�rcio (ALC) de Boa Vista e Bonfim**, que � um grande diferencial competitivo, similar � ZFM ou ALCA (Amap�).
      
      **O que oferece:**
      <ul>
        <li>**Isen��o de ICMS:** Na entrada de produtos estrangeiros (importa��o) para comercializa��o ou industrializa��o dentro dos limites da ALC.</li>
        <li>**Isen��o de ICMS:** Nas sa�das internas de produtos industrializados na ALC (com insumos estrangeiros).</li>
        <li>**Suspens�o do IPI:** Na importa��o de produtos.</li>
      </ul>
      Para usufruir, a empresa precisa estar devidamente cadastrada na SUFRAMA e localizada nas �reas delimitadas (Boa Vista ou Bonfim).`;
    }

    if (texto.includes('st') || (texto.includes('substitui��o') && texto.includes('roraima'))) {
      return `O **ICMS-ST em Roraima** segue as regras gerais dos conv�nios CONFAZ.
      
      **Pontos de Aten��o:**
      1.  **MVA/IVA-ST:** Roraima utiliza prioritariamente as MVAs (Margens de Valor Agregado) definidas nos conv�nios e protocolos. Na aus�ncia, aplica-se a MVA Ajustada.
      2.  **Ressarcimento:** O processo de ressarcimento do ICMS-ST � regulamentado pelo RICMS/RR.
      3.  **Benef�cios da ALC:** A ST *n�o* se aplica a muitas opera��es destinadas a empresas localizadas na ALC, desde que os produtos sejam para comercializa��o ou industrializa��o dentro da �rea incentivada.`;
    }

    if (texto.includes('al�quota') || texto.includes('aliquota interna')) {
      return `A **Al�quota Interna** padr�o do ICMS em Roraima � de **20%** (a partir de 2024).
      
      **DIFAL (Diferencial de Al�quotas):**
      No c�lculo do DIFAL (para consumidor final n�o contribuinte, EC 87/2015), utiliza-se esta al�quota interna.
      
      **Exemplo:** Venda de SP (12%) para RR (20%) para consumidor final:
      <ul>
        <li>O DIFAL devido a Roraima ser� de **8%** (20% - 12%).</li>
      </ul>
      
      Roraima **n�o** possui um Fundo de Combate � Pobreza (FECOP) adicionado ao ICMS, diferentemente da maioria dos outros estados.`;
    }

    // Resposta Padr�o (corrigida)
    return `Sou o especialista em <strong>ICMS de Roraima</strong>.
      <br><br>
      Posso ajudar com:
      <ul>
        <li>Legislação estadual, decretos, protocolos e conv�nios do estado;</li>
        <li>Benef�cios fiscais (ALC Boa Vista/Bonfim), incentivos regionais e programas de desenvolvimento;</li>
        <li>Substitui��o tribut�ria, diferencial de al�quotas e opera��es interestaduais;</li>
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente ICMS - RR</strong><br><br>
