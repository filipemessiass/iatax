/**
 * ============================================
 * AGENTE DOUTRINA JURISPRUD�NCIA - SCRIPT PRINCIPAL
...
(() => {
   ...
   */
  const CONFIG = {
    agentId: 'doutrina-jurisprudencia',
    agentName: 'Agente Doutrinadores - Jurisprudência',
    apiContext: 'DOUTRINA_JURISPRUDENCIA',
    // Mensagem usada ao fechar (retirada do .js original)
    closingMessage:
      'Conversa encerrada. Volte quando quiser cruzar doutrina com precedentes do CARF, STJ e STF.'
  };
  ...
  const BIBLIOGRAFIA = [
    {
      ...
      obra: 'Jurisprudência aplicada ao direito tribut�rio',
      destaque: 'Desembargador do TRF4, conecta teoria e julgados recentes dos tribunais superiores.'
    },
    {
      autor: 'Fernanda Kotzias e Carlos Henrique de Oliveira',
      obra: 'Forma��o do contencioso administrativo no CARF',
      destaque: 'Ex-conselheiros do CARF, oferecem vis�o interna sobre julgamentos e tend�ncias.'
    }
  ];

  // Lista de placeholders (do JS original)
  const PLACEHOLDERS = [
    'Como Leandro Paulsen interpreta a tese do ICMS na base do PIS/Cofins?',
    'Quais cap�tulos de Kotzias tratam do voto de qualidade?',
    'Sugest�es para preparar memoriais com base em precedentes?'
  ];
  ...
  function adicionarMensagemInicial() {
    ...
    const mensagem = `Salve! Vamos alinhar <strong>doutrina e jurisprud�ncia tribut�ria</strong>.
      <br><br>
      <ul>
        ${listaAutores}
      </ul>
      Auxilio na busca de precedentes, na prepara��o de memoriais e no cruzamento com doutrina. Qual tese voc� deseja analisar?`;
  ...
  function adicionarMensagem(tipo, conteudo) {
    ...
    // *** �CONE ADAPTADO PARA JURISPRUD�NCIA ***
    avatarIcon.className = tipo === 'bot' ? 'fa-solid fa-balance-scale' : 'fa-solid fa-user';
  ...
  function mostrarIndicadorDigitacao() {
    ...
    avatarDiv.className = 'message-avatar';
    const avatarIcon = document.createElement('i');
    avatarIcon.className = 'fa-solid fa-balance-scale'; // �cone de Jurisprudência
    avatarDiv.appendChild(avatarIcon);
  ...
  function gerarRespostaBot(mensagem) {
    ...
    // Corrigindo a codifica��o da l�gica de resposta (ex: A?A?)
    if (texto.includes('paulsen') || texto.includes('stf') || texto.includes('stj')) {
      return `**Leandro Paulsen** analisa decis�es do STF e STJ, contextualizando fundamentos constitucionais, modula��o de efeitos e impactos para compliance. Ideal para preparar memoriais e sustenta��es orais.`;
    }

    if (texto.includes('kotzias') || texto.includes('carf') || texto.includes('voto de qualidade') || texto.includes('oliveira')) {
      return `**Fernanda Kotzias** e **Carlos Henrique de Oliveira** explicam bastidores do CARF, forma��o de c�maras e crit�rios de julgamento. Trazem estudos sobre voto de qualidade, empate pr� contribuinte e estrat�gias de defesa.`;
    }

    if (texto.includes('memorial') || texto.includes('estrat�gia') || texto.includes('precedente')) {
      return `Combine os quadros de Paulsen com as notas internas de Kotzias/Oliveira para montar memoriais consistentes. Estruture tese, doutrina, jurisprud�ncia e riscos de forma objetiva para tribunais.`;
    }

    // Resposta Padr�o (Fallback)
    // Se o bot n�o entender, ele se apresenta novamente.
    return `Envie o tribunal (CARF, STJ, STF) ou tese em an�lise e apresento refer�ncias doutrin�rias e precedentes recomendados.`;
  }
    ...
    erroDiv.innerHTML = `
      ...
      <div class="message-content">
        <div class="message-bubble" style="background: #FFFFFF; box-shadow: 0 12px 32px rgba(0, 0, 0, 0.06); border-color: #7AC143; color: #0F0F0F;">
          <strong>?? Erro ao iniciar o agente Jurisprudência</strong><br><br>
